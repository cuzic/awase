...
使い方: effect_learning.py <walk.log> [<walk2.log> ...]
"""
import re
import sys
from collections import Counter, defaultdict

KEY_RE = re.compile(r"^\[[\d:.]+Z\] KEY .*? vk=0x([0-9A-Fa-f]+) .*\((auto)\)")
SNAP_RE = re.compile(
...
def parse(path):
    ...
    return [
        (r["vk"], r.get("before"), r.get("a400"), r.get("a1500"))
        for r in rows
        if r["vk"] in NAMES
    ]
