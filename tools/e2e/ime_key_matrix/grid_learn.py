...
def snap_tuple(line):
    ...
    for mm in (conv_b, conv_a):
        if mm and mm.group(1) != "?":
            ...
            break
    return dict(
        open=None if st is None else st[0],
        conv=raw_conv,
        composing=None if st is None else st[2],
...
def parse(path):
    ...
    for line in open(path, encoding="utf-8", errors="replace").read().splitlines():
        ...
        if m:
            ...
            continue
        m = PRE.match(line)
        if m and m.group(1) in trials:
            trials[m.group(1)]["pre_checks"].append((int(m.group(2)), int(m.group(3)), m.group(4) == "true"))
            continue
        m = SKIP.match(line)
        if m and m.group(1) in trials:
            trials[m.group(1)]["skipped"] = True
            continue
        m = KEYTAG.match(line)
        if m and m.group(1) in trials:
            cur = trials[m.group(1)]
            continue
        if line.startswith("["):
...
def diff(keys_json, imm_json):
    a, b = json.load(open(keys_json)), json.load(open(imm_json))
    def top(c):
...
def main(argv):
    ...
    if sk:
        ...
        for st, n in sk.most_common():
            print(f"  {st}: {n}件")
    if out_json:
        json.dump({f"{s}|{k}": dict(c) for (s, k), c in sorted(cells.items())}, open(out_json, "w"), ensure_ascii=False, indent=1)
