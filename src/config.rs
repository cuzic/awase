...
pub struct GeneralConfig {
    ...
    /// 呼び出しごとに自動ローテーション（baseline/split/self-echo/
    /// fake-ctrl-3event/fake-ctrl-4event）する。加えて Alt 押下中の物理
    /// 半角/全角キーを `KanjiToggleStrategy` 単独発火の診断トリガーとして
    /// 扱う（ADR-133 D0-3）。調査終了後は設定・実装ごと削除すること。
