...
pub struct GeneralConfig {
    ...
    ///
    /// **既知の問題（2026-09-08、dragonflyg4実機検証）**: `"on"`（belief
    /// OFF→ON昇格、ケース2）は実機で「@」再現なしを確認済み。一方
    /// `"off"`（belief既にOFFのまま維持、ケース3）は実機で「@」が
    /// 再現し続けることを確認した——生キーのSuppressと代替
    /// `VK_IME_OFF`の同期送信はいずれも設計どおり発火しているにも
    /// かかわらず再現するため、原因は未解明のまま
    /// （`docs/known-bugs.md` BUG-113節「新規発見」参照、本ADRの
    /// コードとは無関係な既存機能`Ctrl+無変換`でも同条件下で「@」が
    /// 再現することを確認しており、develop側の未解明の問題である
    /// 疑いが強い）。**`"off"`方向は現時点では推奨しない**。
    #[serde(default)]
