...
2026-09-09時点でこの条件は**未達成**——ADR-159段階2（`send_input_safe`/`send_ime_control`の
実際の送信内容を記録するシャドー実行）はまだ実装されておらず（TF2の完了内容参照）、
削除・統合の実例も無い。この条件が満たされるまで、本ルールは参考文書のまま強制しない。
