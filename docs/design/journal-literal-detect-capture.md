...
> **注記（2026-08-23）**: 本文中の「256KiB」は執筆時点の
> `bug_report::LOG_EXCERPT_MAX_BYTES` の値。実運用での問題を受け後日
> 200KiBに引き下げられた（[ADR-095](../adr/095-tray-bug-report-cloudflare-intake.md)
> 「実運用での調整」節参照）。以下の試算（件数・秒数換算）は比率を保ったまま
> 縮小するのみで、判断（レーン容量・予備枠比率を変更しない）自体は変わらない。
