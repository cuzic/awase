...
**正しい対処の方向（未実装）:** 遮断ではなく解釈の修正。foreign-injected な IME モード
キーを `kp_stage_shadow_ime_toggle` で `UserIntentSource::PhysicalImeKey`（ユーザー意図）
に昇格させず、観測（`ObserverReported` 相当）として扱う。OS への配送は維持する。
そのためには hook → RawKeyEvent に injected フラグの伝搬が必要。
