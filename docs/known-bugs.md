...
## BUG-90: PowerToys「マウスなしでコンピューターを制御」(Mouse Without Borders) 使用中に物理「英数」キーが効かない（「かな」は効く、**追補で根本原因を特定・修正**、ADR-118参照）
...
### 追補（2026-09、GitHub Issue #136、根本原因を特定・修正、ADR-118）
...
**修正（決定1+決定4、詳細は[ADR-118](adr/118-injected-and-relay-key-consumption-invariant.md)
参照）:**
