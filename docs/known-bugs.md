...
## BUG-68: `classify_conv_transition` の BUG-26 回復分岐が「持続する変換モード」を「IME 再オープン」と誤読し、明示 IME OFF 直後にタイピングのたびに VK_IME_OFF 再送を無限に繰り返す
...
**原因（コード確認済み、`crates/awase-windows/src/state/conv_classify.rs`
`classify_conv_transition`）:** `IMC_GETCONVERSIONMODE` の `NATIVE` ビットは
「変換モードの好み」（ひらがな/カタカナ等のどれで変換するか）であり、IME の
開閉状態とは独立した軸である。MS-IME は `VK_IME_OFF` で IME を閉じても
この `NATIVE` ビットをクリアしない（本ログでも `conv=0x00000009` は
Ctrl+無変換 の前後で一切変化していない）。

BUG-26（2026-07-17）は「`ConvModeMgr` が focus 変更前から同じ conv 値を
保持しているせいで `conv_mode_changed` が一度も `true` にならず、engine が
本当に ON に戻るべき場面で永久に `EngineSync::None` を返し続ける」不具合を
直すため、`has_native && !effective_open` を **`conv_mode_changed` を問わず
無条件で** `ReportOpenInference(NativeToggleShadowOff)` に倒す設計にした
（`conv_classify.rs` の同分岐コメント参照）。この無条件化が、`NATIVE`
ビットが「開閉と無関係に持続する」という上記の性質と組み合わさると、
**ユーザーが正当に Ctrl+無変換 で IME を閉じた直後は `has_native &&
!effective_open` が構造的に恒久成立し続ける**（NATIVE ビットは消えず、
`effective_open` はユーザーの意図通り false のまま）ため、タイピングを
続ける限り idle-conv-check のたびに「IME が再び開いた」という偽の
`ReportOpenInference` を出し続けることになる。

さらに `kp_apply_conv_engine_sync`（`runtime/key_pipeline.rs`）はこの分岐で
`schedule_ime_refresh(20)` を明示的に叩く実装になっている——これは BUG-51
（TsfNative では explicit_intent 確定後に通常の `TIMER_IME_REFRESH` が
恒久停止するため、ここで蹴らないと乖離が無期限検出されない不具合）への
対処として必要な仕様だが、本バグの文脈では「偽の観測 → 20ms 後に drift
correction 起動 → `VK_IME_OFF` 再送 + `composition marked cold` →
次の打鍵で偽の観測が再生成」という自己持続ループの燃料になっていた。

**修正:** `classify_conv_transition` に `explicit_off_intent: bool`
引数を追加し、`has_native && !effective_open` による回復判定
（`None` 分岐・`conv_mode_changed` 分岐の両方）に `&& !explicit_off_intent`
を足した。呼び出し元（`kp_stage_idle_conv_check` → `apply_idle_conv_check`、
`runtime/key_pipeline.rs`）は `PlatformState::explicit_intent() ==
Some(false)`（`last_intent`、ユーザー操作のみが設定・`FocusChanged` で
クリア）から算出して渡す。

BUG-26 の steady-state 回復自体（`explicit_off_intent=false` の場合）は
変更していない——FocusChanged 直後に engine が本当に ON へ戻るべき場面で
`conv_mode_changed` が立たない既存の問題は従来どおり救済される。
`explicit_off_intent=true` の間だけ、この特定の分岐を「ユーザーの明示意図を
信頼して黙る」方向に倒す。BUG-19 が確立した「観測は `desired_open` を直接
書き換えない」設計はそのまま（本分岐はそもそも `ReportOpenInference` であり
`desired_open` を触らない）——本修正は「観測を生成すること自体」を止める点が
新しい。

**テスト:** `state/conv_classify.rs` に単体テスト2件を追加
（`native_toggle_requires_no_explicit_off_intent`、
`explicit_off_intent_suppresses_native_toggle_shadow_off_steady_state`
——後者が本バグの再現条件そのもの: steady-state・conv 不変・
explicit_off_intent=true で `EngineSync::None` を固定）。独立オラクル
`exhaustive_classify_conv_transition_matches_independent_oracle` の
入力空間に `explicit_off_intent` の次元を追加し 320→640 通りへ拡張、
`oracle_engine`/`oracle_transition` にも同じガードを本番コードと独立に
書き起こして全数一致を確認した。`ConvClassifyFixture`/
`JournalEntry::ConvClassifyCall` に `explicit_off_intent`
（`#[serde(default)]`）を追加し、既存 JSON フィクスチャは `false`
（従来どおり無条件回復）として読み込まれることを確認した。

`cargo test -p awase-windows --lib`（389件）・`--test journal_replay`
（1件）・`--test architecture_guard`（32件）・`--test golden_scenarios`
（22件）・`--test layer_boundary_guard`（8件）、全緑。`cargo xwin check
--tests --target x86_64-pc-windows-msvc -p awase-windows`・`cargo xwin
clippy -p awase-windows --target x86_64-pc-windows-msvc -- -D warnings`
警告ゼロ。**Windows 実機での確認は未実施**——次回、Ctrl+無変換 で MS-IME を
閉じた直後に Windows Terminal でタイピングを続け、`[idle-conv-check]
... reason=NativeToggleShadowOff` および `drift_correction_blind` の
連続発火ログが出ないこと、`ime_on=false` が乱れないことを確認すること。

**関連:** BUG-19（観測が `desired_open` を直接書き換えない設計の由来）、
BUG-26（本バグの原因になった無条件回復分岐そのものの導入）、BUG-43
（GJI/Blacklist 側の同型 drift correction 無限再送、`ir_apply_drift_correction`
の別経路）、BUG-45 追補（同じ「IME OFF Engine ON」症状タイトルでの過去
インシデント、IntentStore による `desired_open` 保護——本バグはその手前の
観測生成自体が誤っていたケース）、BUG-51（`schedule_ime_refresh(20)` 明示
キックの導入経緯）、[ime-belief-architecture](../.claude/rules/ime-belief-architecture.md)、
[fix-requires-evidence](../.claude/rules/fix-requires-evidence.md)。
