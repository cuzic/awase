...
1. **GJI 経路が完全に未検証。** entry は GJI の既存 TSF warmup 経路
   （`crate::tsf::send::send_vk_dbe_alpha_warmup(Charset::HankakuAlpha)`）を
   流用しているが、この関数が「Shift を物理的に押している最中」の文脈で
   scan 付き注入されるのは今回が初めてで、副作用は未検証。
