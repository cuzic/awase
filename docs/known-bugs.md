...
**修正（BUG-08 防御の一般化）:** `hook.rs` の foreign-injected swallow を VK_KANA
限定から **IME モードキー全般（`ImeKeyKind::from_vk` が Some の VK: 0x15/0x16/0x17/
0x19/0x1A/0xF0-0xF4）** に拡張。LLKHF_INJECTED 付きかつ awase 自身のマーカーなしの
IME モードキーは INFO ログを残して swallow し、エンジンにも OS にも通さない。
物理押下（フラグなし）は従来どおりユーザー意図として扱われる。注入がフラグなし
（ドライバレベル）だった場合はこの防御をすり抜けるため、`injected=` ログで次回
発生時に判別する。

**トレードオフ:** タッチキーボードや他ツール（AutoHotkey 等）が SendInput で送る
IME モードキーも swallow される。BUG-08 の VK_KANA swallow と同じポリシー。

**関連ファイル:** `hook.rs`（swallow 一般化 + injected ログ）、
`runtime/key_pipeline.rs::kp_stage_shadow_ime_toggle`（誤読された経路）、
`vk.rs::ImeKeyKind`
...
| `[hook] foreign-injected IME-mode vk=0xXX ... を swallow` | 外部注入 IME モードキーの遮断（BUG-08/BUG-14 防御が発動） |
