...
1. **注入イベントの分類規則**: 学習プロセスは自分の`SendInput`に専用の目印を付け、
   ...
   探す規則では後2つを見落とす）。
2. **キーを伴わない書き込みの検出**: `WM_IME_NOTIFY`（`IMN_SETOPENSTATUS`/
   `IMN_SETCONVERSIONMODE`）とTSF compartment変更通知を監視し、自分で注入していない
   期間にこれらが届いたら外部からの書き込みとして扱う。
...
4. **フックの生存確認とメッセージポンピング**: `ImeDriver`の6メソッドの待ちはすべて
   ...
   `LowLevelHooksTimeout`超過でフックが通知なしに外れる）。学習プロセスは自分の注入
   1件ごとに、自分のフックで自分の目印付きとして観測されることを確認し、観測されな
   かった注入が1件でもあればセッションを失敗にする。
5. **TSF通知経路の生存確認**: COMの戻り値では`advise sink`が外れたことを検出できない
   ため、フックと同じ方式にする——自分の注入で`read_status`により開閉・変換モードの
   変化が観測されたのに対応する通知が届かなかった場合、通知経路が停止したとみなし
   セッションを失敗にする。
6. **残余リスクの緩和**: IMMを直接呼ぶ外部書き込みが測定の窓の中に入った場合は検出
   ...
   場合はその試行を無効にする。
7. **セッション中の監視**: 測定と測定の間の待ち時間に1・2が発生したら、その試行を
