...
**追記（2026-09-15、`wScan=0`仮説をクリーンな条件で再検証・反証を確定、
BUG-033のTsfNative再発調査から波及）**: BUG-033（Chrome/`Imm32Unavailable`
向けのraw-tsf-literal give-up回復）の2026-09-14追補で、TsfNative
（Windows Terminal、`Windows.UI.Input.InputSite.WindowClass`）でも
give-up回復パス`output/probe_io.rs::send_chrome_gji_reinit_and_poll`が
「@」の残留を伴って発火する事例が見つかった。この関数は本BUGの主犯である
`ime.rs::send_ime_mode_key`と同じ`wScan=0`固定（`tsf/output.rs::
make_key_input_ex`）で`VK_IME_OFF`→`VK_IME_ON`を送っており、機構的には
本BUGと同一のトリガー形状（scan=0のIMEモードキー単体SendInput）を
別経路から踏んでいる可能性が高いと判断した。

これを機に、エントリ20（2026-09-05）で「`wScan=0`→実scan送信への変更は
『@』に変化なし」として一度反証・revertされていた仮説を再検証した。
エントリ20の否定的結果は、**同じ試行中に無関係な別バグ（BUG-114、
drift correctionの`VK_IME_OFF`無限に近いバースト）が同時発生しており、
「実scanにしても効かない」のか「バーストという交絡因子にかき消された
だけ」なのかを当時は区別できていなかった**（BUG-114は後日独立に修正
済み、詳細はdocs/experiments.mdエントリ20参照）。BUG-114修正後、
実scan送信を単発クリーンな条件で再テストしたことは一度もなかったため、
今回`spike/bug033-realscan-ime-mode-key`ブランチで`send_ime_mode_key`と
`send_chrome_gji_reinit_and_poll`の両方の`make_key_input_ex`（`wScan=0`
固定）を`make_scan_key_input`（`MapVirtualKeyW`実測scan、`KEYEVENTF_
SCANCODE`は付与しない）に差し替え、dragonflyg4実機（Windows Terminal
+ PowerShell + GJI）で物理半角/全角キー単独タップを実施した。

**結果: 実scan送信でも「@」は再発した**（ユーザー報告:
「あいうab@cあいう」——「あいうabcあいう」と打ったつもりが`b`と`c`の間に
「@」が混入）。これにより、**`wScan=0`固定は「@」の必要条件ではない
ことがクリーンな条件で確定した**。エントリ20時点で疑われていた
「BUG-114のバーストに紛れて否定的結果になった」という可能性は排除され、
`wScan`の値そのものは真因ではないという結論がようやく確定的になった
（VK値・`SendInput`バッチ形状に続き、scanコードも機構から除外された）。

この結果は、BUG-033のTsfNative「@」再発の根本原因が
`send_chrome_gji_reinit_and_poll`の`wScan`にあるのではなく、
本BUGが最終的に確立した「GJI自身のTSFキー横取り」「beliefが変化しな
くても強制actuateすること自体がトリガーになりうる」という、より深い
（GJI内部の、クローズドソースの）機構によるものだという見立てを
補強する。スパイクブランチ（`spike/bug033-realscan-ime-mode-key`）は
本結果の記録をもって役目を終えたため破棄する——マージ対象のコードは
含まない。
