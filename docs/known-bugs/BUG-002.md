...
> **注(2026-09-20、ADR-193で発覚): 以下の「現在の対策」は古い(stale)。** 下表の`CHROME_PROBE_*`定数と
> F2事前送信・probe事前待機は、2026-07-18のBUG-024対応で物理削除され、per-VK confirmに一本化された
> (`tuning.rs`に存在しない)。cold種別の判定は`gji_idle_ms`の7s/10s(`ColdKind::classify`)で、下表の「>10s」は旧記述。
> 2026-09-21の実機確認（GJI、`chrome_probe --settle=3000〜14000`、有効probe20件）では**再現しなかった**（ADR-193）。現在の対策は per-VK confirm。

**現在の対策(削除済み・当時の記録):** `probe_min_ms` を以下の2段階で切り替える。
