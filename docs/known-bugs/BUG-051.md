...
related_adr: ["ADR-84", "ADR-86", "ADR-87", "ADR-89", "ADR-90", "ADR-171"]
---
...
**追補（2026-09-13、別プロファイルでの未修理な再発を確認。同日中に因果帰属を
訂正）:** BUG-141（`ADR-171`）の調査で、`runtime/mod.rs::
reschedule_ime_refresh`の「`explicit_intent().is_some()`の間は
`TIMER_IME_REFRESH`の次回tickを張らない」という同一の構造欠陥が、
**Imm32Unavailable×GJI**（msedge.exe）でも起きていることをapp.log実測で
確認した（report `01M2D8HS5SBWSZ221P240Z4ZXE`、absolute
`2026-09-13T11:30:03.35`〜`11:30:14.25`の区間、tickはすべてapply直後の
20-50msイベントkickで500ms周期tickが1つも無い）。上記の修正（`21ca84d1`）は
**idle-conv-check（TsfNative）という1つの観測サイトにしか入っていない**ため、
`fix_commits`に載っているからといって「解決済み」と誤って結論づけないこと。
`Imm32Unavailable`側の同種の再起動漏れは未修理のまま残っている。

**訂正（同日、opus-adversarial-consult round4で実ログ再検証）**: 当初
「この停止によりdrift correctionの周期チェーンが一度も回っていなかった」と
記述したが過大な帰属だった。実際には`[stage-observe]`ログ
（`ir_stage_observe`冒頭で無条件に出る）が当該区間に5回出ており、
`ir_apply_drift_correction`自体は最低4回実行されている。**補正が出なかった
近因は本stallとは別に2つある**: (1)打鍵が続く間`strategy=SkipTyping`で
観測を一切書かない（`idle_ms < TYPING_IDLE_MS`=500ms、Blacklist/TsfNativeは
`explicit_verify`の対象外）、(2)Blacklistで実際に走ったtickでも
`observer_poll=None`（`observer/gji_observer.rs`のpredates-focusガード）。
本stall（500ms周期tickが無いこと）自体は実在し訂正不要だが、
「補正が出なかった」ことの説明としては(1)(2)が主因であり、
BUG-051の対策（タイマーを蹴る）だけでは(1)(2)は解消しない。
（案Z自体はこの穴・上記(1)(2)のいずれにも依存しない別の直接原因の修正であり、
これらはすべて案Z実装後もawaseの「送ったOFFが実際に効いたかの自動確認・
自動収束」を機能しないままにする残存リスクとして残る。）
