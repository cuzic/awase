...
**追補（2026-09-13、別プロファイルでの未修理な再発を確認）:** BUG-141
（`ADR-171`）の調査で、`runtime/mod.rs::reschedule_ime_refresh`の
「`explicit_intent().is_some()`の間は`TIMER_IME_REFRESH`の次回tickを張らない」
という同一の構造欠陥が、**Imm32Unavailable×GJI**（msedge.exe）でも起きている
ことをapp.log実測で確認した（report `01M2D8HS5SBWSZ221P240Z4ZXE`、absolute
`2026-09-13T11:30:03.35`〜`11:30:14.25`の約11秒間、`[stage-observe]`ログが
`explicit_intent=Some(false)`の間`observer_poll=`行を一切出さない）。上記の
修正（`21ca84d1`）は**idle-conv-check（TsfNative）という1つの観測サイトにしか
入っていない**ため、`fix_commits`に載っているからといって「解決済み」と
誤って結論づけないこと。`Imm32Unavailable`側の同種の再起動漏れは未修理のまま
残っている（案Z自体はこの穴に依存しない別の直接原因の修正であり、この穴は
案Z実装後もawaseの「送ったOFFが実際に効いたかの自動確認・自動収束」を
機能しないままにする残存リスクとして残る）。
