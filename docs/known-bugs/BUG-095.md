...
**2026-09-12 追記: 別ユーザーの新規報告で症状1相当の真因を特定（BUG-140参照）。**
`keys.ime_detect.on`に`right_thumb_key`と同じキーが登録されている場合に限り、
変換キー単独タップ毎にIME再適用（`SyncKey`）が暴発しGJI自身の変換機能と競合する
ことをjournalで実測確認した。ただしこれは全ユーザー共通の一般バグではなく、
この特定の設定組み合わせ時のみ発生する。上記「eager path」仮説は当該報告
（`01M13EACMQ7D2VETW75N0BTZ9C`）のconfig.tomlに`ime_detect.on`の変換キー登録が
無かったため無関係のままの可能性が高いが、再検証はしていない。
