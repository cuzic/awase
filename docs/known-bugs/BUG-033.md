...
**2026-09-14追補: TsfNativeプロファイルでの未修理の再発（ADR-171実機A/B検証中に発見）**:
ADR-171（BUG-141修正、`gji_direct_already_matches`への`candidate_was_seen`配線）の実機A/B
検証中、UWP系入力欄（`profile=TsfNative`、class`"Windows.UI.Input.InputSite.WindowClass"`、
mode=Tsf、VK Sequential経路）で本バグと同一の`giving up`経路が発生した。「あいう」を
繰り返し入力する実験で: cold=55（1回目、`raw TSF literal suspected → backspace ×1 +
re-send "ko" scheduled`で正常復旧）→ cold=56（2回連続、`consecutive raw-tsf-literal
(count=2) → giving up, backs=1 cleanup only (no re-send)`）で、視認上「あ」「い」が
「@」（その物理キー位置のJIS配列上の生文字と推定、未確認）に化けて残留した
（ユーザー報告: 「あいうあいうabcあいうabcあいうabcあいう@@いう」）。

本ファイル冒頭の修正は「Chrome/`Imm32Unavailable`」を対象に実機ソークしたもので
（上記2026-07-22ソーク結果の節）、`send_chrome_gji_reinit_and_poll`によるgive-up後の
GJI ON再送がTsfNativeプロファイルでも同様に効くかは当時未検証のまま持ち越されていた
（同ソーク結果の最後の段落が予告していた「次回give-upが実機で発生した際にログで確認
すること」が今回に該当）。今回のログでは`schedule_chrome_gji_reinit`自体は呼ばれて
いたが、その後に「@」が視認上残ったため、reinit後もgive-up時点の描画崩れ（backspace
1回では化けた文字を完全には消せない、または再送が無いため元のかな文字が最終的に
一度も出力されない）自体は解消されていない可能性がある。ADR-171の変更（`ime_controller.rs`
のGjiDirect OFF方向・`state/ime_actuation_decision.rs`）はこの経路（`output/probe_io.rs`・
`tsf/warmup/probe_fsm.rs`）を一切触れておらず無関係——ADR-171側のバグではないことを
確認済み。根本修正は未着手（TsfNativeでのgive-up後cleanupの完全性検証、または
backspace本数の見直しが次のアクション）。

**追記（2026-09-15、機序をBUG-113系統として再分類、backspace本数説は
優先度を下げる）**: 上記追補は「give-up後のcleanup（backspace本数）の
不備」を次のアクションとしていたが、ユーザー指摘を受けて調査した結果、
より説明力の高い候補が見つかった。`send_chrome_gji_reinit_and_poll`
（give-up回復用に`VK_IME_OFF`→`VK_IME_ON`を送る関数）は
[BUG-113](BUG-113.md)の主因である`send_ime_mode_key`と同じ`wScan=0`
固定（`tsf/output.rs::make_key_input_ex`）でこの2キーを送っている。
BUG-113はWindows Terminal + GJI（+ PowerShell/PSReadLine）で「scan=0の
IMEモードキーSendInputを1回送るだけで『@』が出る」ことを実機A/Bで
既に確定済みであり（`giving up`の判定精度やbackspace本数とは無関係に、
**回復策自体が「@」の引き金になりうる**）、本追補が観測した「@」残留の
より有力な説明になる。

検証のため`spike/bug033-realscan-ime-mode-key`ブランチで`wScan=0`を
実scan送信（`make_scan_key_input`）に変える診断スパイクを実施したが、
**dragonflyg4実機で「@」は再発した**（詳細は[BUG-113.md](BUG-113.md)の
2026-09-15追記・`docs/experiments.md`エントリ20追記参照）。したがって
「`wScan=0`が原因」という仮説自体もこの機会に反証されており、
`send_chrome_gji_reinit_and_poll`側の対策としてscan修正は候補から
除外してよい。根本原因はBUG-113が最終的に到達した「GJI自身のTSFキー
横取り」「beliefが変化しなくても強制actuateすること自体がトリガーに
なりうる」という、より深い（クローズドソースの）機構に起因する可能性が
高いという見立てのまま、次のアクションは未着手。backspace本数見直しの
優先度は下げる（原因ではなさそうなため）。
