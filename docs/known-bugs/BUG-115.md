...
**追記(2026-09-20、ADR-187)**: `gji_thumb_key_ime_toggle`の既定を`false`→`true`に変更した。`false`だとATOK利用者は生の無変換/変換がGJIに届き実IMEは開閉するがEngineが追随しない(CIの実機E2Eで3/3再現)。Toggleは非冪等なのでawaseが唯一の変更主体としてactuateする方針(方向固定のOn/Offはパススルー+followのみ)。明示`false`はopt-outとして尊重される。
