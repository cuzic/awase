...
**申し送り（2026-09-06、ADR-146決定10より）**: GJI(Mozc)専用の軽量な
代替ツール（[ADR-146](146-gji-keymap-swap-tool.md)、GJI自身のキーマップ
書き換え）が並行して起票されている。本ADRの設定GUI実装に着手する際は、
ADR-146の「GJIキー入れ替え」タブとの併存関係（両方のUIが並ぶことに
よる混乱、両方設定された場合の合成挙動）を必ず再検討すること。
