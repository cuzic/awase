...
2. 内訳確定後、#6（`apply_force_on_for_imm_broken`）側の同型修正
   ...
   として維持しつつ、#6側の修正を優先する。**2026-09-15、ADR-172が
   このタスクを引き継いだ——2ラウンドの敵対的レビューを経て#6側の
   実装案がいずれも実装不能と判明したため、当面「コード変更なし」で
   確定し、内訳確定（次のアクション1）だけをADR-172側の「次のアクション」
   として引き継いでいる。本項目はADR-172に委譲済みとしてクローズする。**
