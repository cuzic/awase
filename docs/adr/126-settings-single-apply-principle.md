...
- **キーボードモデル不一致に関するガードを、目的の異なる2つの独立したガードに
  ...
  **この適用でキーボード配列が実際に変わるかどうかで分岐させる**（round5
  ...
  - 一致する場合（＝この適用ではキーボード配列を変更していない）:
    **`apply_confirmed()`は中止せず**、`self.status`に「既定の配列ファイル
    `{default_layout}`を読み込めません（{パースエラー}）。awaseエンジンは
    この配列を読み込めない状態です」という警告を併記して保存処理を続行する。
    既に壊れている`.yab`をたまたま報告するだけで、この適用自体は新しい
