...
| [157](157-symmetric-target-resolution-for-drift-correction-and-force-on.md) | BUG-110追補7（issue #189、MS-IME/Chrome）で、追補3が確定した`check_drift_correction()`（生の`desired_open()`）×`apply_force_on_for_imm_broken()`（`effective_open()`）の二重SSOTが第3の発火経路でも起きることを確認。追補4で不採用になった「force-ON側だけをwarrantに繋ぐ」非対称適用ではなく、Phase 2（B1×drift correction、追補6）が実証した「両者に同じ生きた判定を共有させる」パターンを対称に適用する案を提案。`IntentStore`優先→観測ベース導出→生フィールドの順で`desired`を解決し直す。`apply_force_on_for_imm_broken`自体の発火条件・`AppliedImeState`更新経路には触れず、ADR-149/151/153のBlocker（`force_on_attempt_allowed`の`applied`詐称問題）とは別軸であることを明記 | 起票・opus-adversarial-consult実施前 |
