...
| [133](133-windows-terminal-vk-kana-scan-code.md) | Windows Terminal + 日本語IMEで`VK_KANJI`/`VK_KANA`押下時に余分な「@」が出る報告への対策設計。v1〜v4はドキュメントレビュー（Sol 2体の敵対的レビュー4ラウンド）で「VK_KANAをVK_DBE_HIRAGANA+scan付き送信へ置換するWindows Terminal限定hidden実験」として収束したが、2026-09-05にdragonflyg4実機で検証したところ前提が崩れた: 物理`VK_KANA`(0x15)はこの機体のフックに一度も到達せず（既にVK_DBE_HIRAGANA(0xF2)としてOSから届く）、D2の置換スパイクは到達不能コードと判明。ユーザーが「VK_KANJI」と呼ぶ半角/全角キーも同様に`VK_KANJI`(0x19)ではなく`VK_DBE_SBCSCHAR`(0xF3)/`VK_DBE_DBCSCHAR`(0xF4)として届き、押下のたびに交互にトグルする。実機で2回連続、`vk=0xF3`(TurnOff)の直後だけ「@」が出現し`vk=0xF4`(TurnOn)では出ないことをタイムスタンプ相関で確認。さらにEngineを明示的にOFFにすると同じキーで「@」が出ないというユーザー報告から、Windows Terminal単体の文字化ではなくawase自身の`GjiDirectStrategy::apply(open=false)`が`send_ime_mode_key(VK_IME_OFF)`を`wScan=0`で送信していることが真因候補と判明（`apply(true)`側は`AlreadyMatched`no-opガードがあり非対称）。dragonflyg4実機で`MapVirtualKeyW`/`ToUnicodeEx`を直接呼び検証したが、いずれのVKも単体では文字を生成せず、Windows Terminal自身の素朴な`ToUnicodeEx`フォールバックでは再現しないことも確認——GJI自身のTSF内部処理（クローズドソース）が真因の可能性が高いところまで絞り込んだ | **BUG-113の真因候補を`VK_IME_OFF`単体`SendInput`バッチに絞り込み済み（2026-09-05）。修正方針（mode=3 fake-ctrl-bracket案）はOpus敵対的レビューで設計してから実装する。** D1〜D6（VK_KANA/VK_KANJI置換）はv1〜v4時点のまま保持し実機検証ログで反証済み。`wScan=0`修正・BUG-114単独説の順に反証を重ねた末、ユーザー指摘（Ctrl+無変換も同じsend_ime_mode_key(VK_IME_OFF)を送るのに「@」が出ない）を機に、Ctrl+無変換は修飾キー押下によりOFF送信が「release→OFF→restore」を1バッチにまとめる形になるのに対し物理半角/全角キー単独では`VK_IME_OFF`単体2イベントのバッチになる、という違いを発見。`GjiDirectStrategy::apply(open=false)`の送信方式を呼び出しごとに自動ローテーションする診断スパイク（4モード: baseline/ImmSetOpenStatus/keystate-clear/fake-ctrl-bracket）を実装し実機で比較したところ、単体バッチ（mode0/2）で「@」が出て、メッセージベースまたは複数イベントバッチ（mode1/3）では出ないことを複数ラウンドで再現確認。検証中にBUG-114のdrift correction暴走が何度も自然発生したため安全弁も追加。ON方向での同条件検証・原因不明の「新規ウィンドウでTurnOff検出不能」現象は未着手のまま次セッションへ持ち越し |
