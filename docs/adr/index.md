...
| [190](190-msime-immcross-failure-fallback-idempotent-vk-ime-on.md) | Microsoft IMEでImmCrossが失敗したとき、非冪等なVK_KANJIトグルでなく冪等なVK_IME_ON/OFF(MsImeDirect)へフォールバックする(BUG-152) | **実装済み(未マージ)・CI実機E2E検証済み**。opus round1〜3で収束+KanjiToggle撤去。windows-build CI・実機未。CI(a8/a9)検証済み、実機未 |
