...
| [125](125-egui-winit-dynamic-ime-association-focus-model-gap.md) | ユーザー報告（不具合報告画面・設定画面でのテキスト入力の重さ・かな文字混入）の調査ADR。最初に検討した`disable_apps`へのプロセス名追加案は「eframe/egui全体が非対応という話になる」とユーザーに却下され、対症療法ではなく機構の特定に転換。`winit`/`egui-winit`の実ソース読解から「ウィジェット単位のフォーカス移動のたびに同一HWNDのHIMCが`ImmAssociateContextEx`で脱着される」という仮説を立てたが、専用スパイク（`spike_egui_himc_reassociation_probe`、clipwire経由でWindows実機実行）による直接観測ではこの脱着は一度も再現しなかった。代わりに、フォーカス中`awase-settings.exe`のHIMCが終始0（NULL）のままという別の事実を発見——同じログ内でIMM32が使えないことが既知のmsedge.exe/WindowsTerminal.exeと同じパターンであり、`AppImeProfile::from_class_name`の既定分類`Standard`（IMM32クロスプロセス制御が使えるはず）が実態と合っていない可能性を示唆する。当初仮説を前提にしたB/C案には進まず、実運用の`awase.exe`でこのウィンドウへのIMM32クロスプロセス制御が実際に機能しているかを確認することを最優先の次のアクションとした | **調査中・設計未確定。当初仮説は実機スパイクで再現せず棄却/保留。実装なし** |
