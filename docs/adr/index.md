...
| [171](171-gji-candidate-reopen-after-off-observation.md) | `gji_direct_already_matches`が候補ウィンドウ再表示(`candidate_was_seen`/`candidate_visible`)を無視して再送を握り潰す不具合を修正(BUG-141)。belief経由の自動補正案はround1/2で計8件のBlockerが出て見送り | 起草・案Z round3反映済み(serde default・送信時ラッチ消費に修正)・round4待ち |
