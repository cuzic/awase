...
| [125](125-egui-winit-dynamic-ime-association-focus-model-gap.md) | ユーザー報告（不具合報告画面・設定画面でのテキスト入力の重さ・かな文字混入）の調査ADR。最初に検討した`disable_apps`へのプロセス名追加案は「eframe/egui全体が非対応という話になる」とユーザーに却下され、対症療法ではなく機構の特定に転換。`winit`/`egui-winit`の実ソース読解により、egui-winitがフォーカス中のテキストウィジェットの有無に応じて毎フレーム`window.set_ime_allowed()`を呼び、winit側がこれを同一HWNDに対する`ImmAssociateContextEx(IACE_DEFAULT/IACE_CHILDREN)`（IMEコンテキストの脱着）として実装していることを特定。awaseのフォーカス追跡はトップレベルHWNDの切替のみを契機にしており、同一HWND内でのこの種のHIMC脱着を検知する経路が存在しないという構造的ギャップを発見。ただし実機ログでの裏取り（allow_ime遷移の実際の頻度、重さ・かな混入の実体がこの機構によるものか）は未実施のため、本ADRは実装方針を確定せず、実機データ収集を次のアクションとして提示するに留める | **調査中・設計未確定（実機未検証）。実装なし** |
