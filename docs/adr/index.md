...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。起草前調査で「かな系キー同士の入れ替え」は物理的に不成立（JIS配列で対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。`from`判定はscan値ベース、`to`がかなスロットになる場合の合成vkは`VK_DBE_HIRAGANA`固定（0xF5/0xF6はBUG-61により構造的に禁止）。r1レビューでBlocker3件を検出・修正: (1) Alt/Win押下中の合成0xF2がMS-IME/GJI問わずBUG-61型の入力方式切替を誘発すると実機診断記録から判明→Alt/Win/Shift押下中は代入自体を新規押下時点で不発火にする制約を追加、(2)(3) 上流の`CACHED_SWALLOW_ALT_KANA_MODE_SWITCH`等の早期returnがかなスロットの保留KeyUpを消費しstuck keyを起こす（既定設定・オプトアウト設定の両方）→swallow分岐にKeyUp注入してからクリアする処理を追加。両エージェント独立発見でTHUMB_KEY_OPTIONSの事実誤認（VK_DBE_ALPHANUMERIC不在・VK_KANA脱落）も訂正、親指キー設定との全面禁止（r0）はADR-141決定4r2と同型の過剰禁止と判明し撤回 | r1（Opus 2体敵対的レビュー1ラウンド、Blocker3件対応済み。r2レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
