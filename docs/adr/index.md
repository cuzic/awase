...
| [103](103-warmup-probe-pending-integrity.md) | Warmup/Probe過渡期のpending取りこぼしとFSM整合性。probe_ioの早期returnがdeferred VIDフラッシュとGjiFsm通知の両方を飛ばす問題(BUG-27未解決follow-up)、gji_fsmのEndCompositionがColdKind/ProbeParamsを固定値で捏造する問題、post_bypass_passthroughフラグのスコープをarmed_pid(プロセス同一性)に変更 | 提案（未実装、2026-08-26） |
