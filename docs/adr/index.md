...
| [137](137-shift-katakana-dbe-mode-key-suppression-regression.md) | ユーザー報告「Shift+かなでカタカナにならない」を発端に調査。当初は「NICOLA小指シフト面（Face::Shift）の設計上そもそも無い機能」と誤診したが、ユーザーの「以前は動いていた」という指摘を受けOpusで`git log`を掘り下げ直し撤回: 真因は`runtime/transport.rs::PhysicalKeyDisposition::plan`がBUG-52修正（2026-08-05, `bdf4a139`→`9a02ce6b`）で`VK_DBE_*`のKeyDownを`shadow_toggled`に関わらず常時Suppressするようにした際、Shift押下の有無を判定に含めなかったため、BUG-52が本来防ぎたかった「Shiftなしでの物理IMEトグルキー連打による誤爆」と「Shift併用によるユーザーの明示的なカタカナ要求」を区別できず両方まとめてSuppressしてしまったリグレッション（BUG-116として起票）。`RawKeyEvent.modifier_snapshot.shift`が既に弁別に使える手がかりとして存在することを確認し、これを`is_dbe_mode_key_down`の追加条件にする案を提示。副産物として`docs/adr/135`の「別件観察」の原因誤診（awase側VK分類は正しくGJI/OS側が原因、という記述）も本ADRで訂正 | **設計ドラフト（Opus敵対的レビュー未実施、未実装）** |
