...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。起草前調査で「かな系キー同士の入れ替え」は物理的に不成立（JIS配列で「かな/カタカナ/ひらがな」に対応する物理キーはscan 0x70の1個のみ、IME状態依存で`VK_DBE_HIRAGANA`/`KATAKANA`/`ALPHANUMERIC`のいずれかとして届く）と判明し、ユーザーとの対話でスコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。`from`判定はvkではなくscan値（0x70）ベースに変更（`RawKeyEvent.scan_code`が役割代入で書き換えられず保持される既存構造を利用）、`to`がかなスロットになる場合の合成vkは`VK_DBE_HIRAGANA`固定（`VK_DBE_ROMAN`/`NOROMAN`はBUG-61の復旧不能リスクにより`to`として構造的に禁止、他の`VK_DBE_*`亜種は`transport.rs`の既存無条件Suppressガードにより機能しないため除外）。全単射モデルを4要素に拡張、hold-state/KeymapLatchはかなスロット専用ペアを追加。既存の`CACHED_SWALLOW_ALT_KANA_MODE_SWITCH`（BUG-08/61/62対策）は挿入点より前段の早期returnのため評価順序の変更は不要と確認。Shift併用時のカタカナ相当挙動は本ADRのスコープ外（既知の制限として明記、将来拡張） | r0（レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
