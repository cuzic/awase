...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。物理的に「かな系キー同士の入れ替え」は不成立（対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。r1「生SendInputでOSへ0xF2を送る」設計→Blocker3件発見・修正。r2「hook.rs側modifierゲート＋KANA_DOWN_WAS_ALLOWED」で対処→改訂自体が新規Blocker2件（全単射を破る非対称ゲート／クロススレッドで実装不能な値）を混入と判明、アーキテクチャを「to=かなはOSへ二重配送せず既存のIME actuation経路に委ねる」設計へ転換。r3でその転換の安全性論拠自体の事実誤認（actuationは`plan()`より前に既に発火している、送出VKがDBE系でないから安全という論拠へ訂正）と`transport.rs::plan`の新条件の欠陥（`!event.injected`・機能有効時のみ適用する`kana_role_active`フラグの欠落、`kp_restore_hiragana_for_suppressed_mode_key`の必要十分条件破壊）を発見・修正。「from=かな」方向（物理かなキー→安全な3キー）は引き続き生SendInputで届くため、上流swallowガードのKeyUp消費対策（既定設定・オプトアウト設定とも、無効化はルール集合全体に訂正）を維持。r4でpremortem役はr3のBlockerをゼロと判定したがarchitect役が新規Blocker1件（`kp_stage_shadow_ime_toggle`のno-op分岐でBUG-10「食い逃げ」救済経路が完全に塞がる）を発見、`ime_will_be_turned_on_elsewhere`（delegate_owned・no-op分岐の状態を見て既存の仕組みがIMEを実際にONにする場合にのみSuppressする）を新設して対応、決定6のthumb_key=ひらがな構成での類似問題(NM10)も同時に解消 | r4（Opus 2体敵対的レビュー4ラウンド、premortem役はr3でBlockerゼロ判定・architect役の新規Blocker1件を解消。r5レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
