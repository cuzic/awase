...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。物理的に「かな系キー同士の入れ替え」は不成立（対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。r1「生SendInputでOSへ0xF2を送る」設計→Blocker3件発見・修正。r2「hook.rs側modifierゲート」で対処→改訂自体が全単射を破る新規Blockerを混入、アーキテクチャを「to=かなはOSへ二重配送せず既存のIME actuation経路に委ねる」設計へ転換（`transport.rs::plan`のF2分岐を役割代入由来では条件付きSuppress）。r3で安全性論拠を「送出VKがDBE系でないから安全」に訂正。r4でBUG-10食い逃げ救済経路の消失を発見、`ime_actuation_will_fire_before`（`delegate_owned`・no-op分岐の状態を見て既存の仕組みが実際にIMEをONにする場合にのみSuppress）を新設。r5でそのフラグ自体のタイミング欠陥2件（ライブ値読み取りで機構全体が死ぬ／KeyUp再評価で毎打鍵オーファンKeyUpが出る）を発見・修正——`kp_stage_shadow_ime_toggle`呼び出し直前のスナップショット化・KeyUp判定からの完全除外・戻り値の構造体化。「from=かな」方向（物理かなキー→安全な3キー）は引き続き生SendInputで届くため上流swallowガードのKeyUp消費対策を別途維持 | r5（Opus 2体敵対的レビュー5ラウンド、各ラウンドで新規混入した指摘を都度解消。r6レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
