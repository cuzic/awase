...
| [195](195-keymap-learn-productization.md) | カスタムキーマップ対応のため、IMEキー効果の学習(awase-keymap-learn)を独立プロセスとして製品化する（設定読取→独立プロセスでの巡回学習→自己検証→永続化→実行時読込→ADR-176統合） | **段階0/1/2/3/4/5/6/8はdevelopマージ済み(2026-09-23、PR #250〜#258)。段階7(安全対策)のみ未着手。** 段階4/6/8はADR-196で置換予定(ADR-196自体は未マージ)、段階3は一部フィールド拡張 |
