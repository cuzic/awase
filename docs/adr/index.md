...
| [187](187-atok-passthrough-mode-key-observed-belief-follow.md) | GJI(ATOK)で無変換/変換をパススルー(opt-in無し)するとき、生キー通過後に実IMEを読み直し、開閉が変わっていれば古い明示意図を無効化して観測に追随させる(予測せず・値を書かず・actuateせず) | **ドラフトv2+round2の結果(未収束、方針判断待ち)**。CIで3/3再現した未解決問題(ADR-186) |
