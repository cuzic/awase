...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。物理的に「かな系キー同士の入れ替え」は不成立（対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。設計はr1〜r8で8ラウンドのOpus敵対的レビューを経て収束: 「to=かな」は生SendInputでOSへ送らず、`transport.rs::plan`のF2分岐を役割代入由来では条件付きSuppressし既存のIME actuation経路に委ねる（安全性論拠は「送出VKがDBE系でないから安全」）。動的条件（`kana_role_active`・`actuation_will_fire`）はfresh press時点でのみ評価し`plan()`呼び出し元のラッチへ確定、auto-repeat/KeyUpはそのラッチを踏襲する（fresh press自体はhook.rsが`is_fresh_press`ビットとして`RawKeyEvent`経由で明示的に運ぶ）。「from=かな」方向（物理かなキー→安全な3キー）は引き続き生SendInputで届くため上流swallowガードのKeyUp消費対策を別途維持。詳細な変遷（各ラウンドで発見・解消したBlocker/Major）はADR本文のステータス節・変更履歴を参照 | r8（Opus 2体敵対的レビュー8ラウンド、各ラウンドで新規混入した指摘を都度解消し収束中。r9レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
