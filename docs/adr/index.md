...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。起草前調査で「かな系キー同士の入れ替え」は物理的に不成立（JIS配列で対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。`from`判定はscan値ベース。r1で「toがかなの場合は合成vk=VK_DBE_HIRAGANAを生SendInputでOSへ送る」設計にしたところ、r1レビューでBlocker3件検出・修正したが、r2レビューでその修正自体（hook.rs側のmodifierゲート、KANA_DOWN_WAS_ALLOWED）が新規Blocker2件（全単射を破る非対称ゲート／フックスレッドとメインスレッドを跨ぐ原理的に実装不能な値）を混入と判明（ADR-141/142で繰り返されたパターン）。r2でアーキテクチャを転換: 「to=かな」はOSへ何も送らず`transport.rs::plan`のF2分岐を常にSuppressし、既存のIME actuation経路（`kp_stage_shadow_ime_toggle`→`ime_controller::apply`）へ委ねる設計に再設計。これによりBlocker群に加えAlt/Shift関連のMajor群も同時に解消。「from=かな」方向（物理かなキー→安全な3キー）は引き続き生SendInputで届くため、上流swallowガードのKeyUp消費対策（既定設定・オプトアウト設定とも）は維持 | r2（Opus 2体敵対的レビュー2ラウンド、r1で新規混入したBlocker2件をアーキテクチャ転換で解消。r3レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
