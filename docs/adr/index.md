...
| [125](125-egui-winit-dynamic-ime-association-focus-model-gap.md) | ユーザー報告（不具合報告画面・設定画面でのテキスト入力の重さ・かな文字混入）の調査ADR。最初に検討した`disable_apps`へのプロセス名追加案は「eframe/egui全体が非対応という話になる」とユーザーに却下され、対症療法ではなく機構の特定に転換。`winit`/`egui-winit`の実ソース読解から「ウィジェット単位のフォーカス移動のたびに同一HWNDのHIMCが`ImmAssociateContextEx`で脱着される」という仮説を立てたが、専用スパイク（`spike_egui_himc_reassociation_probe`）による実機直接観測ではこの脱着は一度も再現しなかった。代わりに見つかった「HIMCが終始0」という事実から「`AppImeProfile::Standard`分類・IMM32クロスプロセス制御そのものが機能していない」という第2の仮説を立てたが、awase本体と同一のWin32呼び出し列（`ImmGetDefaultIMEWnd`+`WM_IME_CONTROL`）を再現した別のスパイク（`spike_egui_ime_control_probe`）による直接検証では、この機構は正常に機能している（有効なIMEウィンドウ・50ms以内の応答・実際のON/OFF操作との一致）ことが確認され、この仮説も否定された。2つの仮説がいずれも実機で否定されたため、原因はこれらの機構の外側（実際に`awase.exe`本体エンジンを動かした状態でのタイミング・同時打鍵判定等）にあると考えられ、実エンジン稼働下での再現・ログ取得を次のアクションとした | **調査中・設計未確定。2つの仮説がいずれも実機スパイクで否定された。実装なし** |
