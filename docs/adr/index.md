...
| [095](095-tray-bug-report-cloudflare-intake.md) | タスクトレイから不具合報告(症状発生時の内部状態を自動添付)を送信する機能。受け口にGitHub Issuesは使わず(添付ログの公開リポジトリ流出を回避)、`report.awase.cc`をCloudflare Workers+R2の非公開受付として新設。GCP Cloud FunctionsはCI基盤との管理主体一本化の利点はあるが、無料枠がカード前提・GCSのエグレス課金・乱用時の実費リスクの点でCloudflareに経済性で劣ると判断し不採用。報告時のAI要約・トリアージは今回スコープ外。`awase.cc`本体(GitHub Pages)は移行せずサブドメイン追加のみで対応 | **提案中**(設計のみ、実装未着手) |
