...
| [122](122-cold-start-per-vk-confirm-race-recovery.md) | BUG-75（既存、`StaleConfirm`回収がGJI I/Oカウンタのポーリング遅延をliteralの証拠と誤認する）の実機再発。不具合報告`01M1JGJNDJT9ZAEMRAEB58ES5A`（LINE+GJI、42秒アイドル後のcold-startでセッション最初の1文字「to」がStaleConfirm誤判定→ESC+全体再送でモーラ重複）から起票。Opus 2体（architect/premortem）敵対的レビューround1で、v1の根本原因記述（BUG-75元報告と本incidentを「同じ猶予不足」と同一視）の飛躍を指摘され訂正、`fencing_active=(last_write_ms!=0)`によりcold-start直後は構造的に必ずstale側から始まる事実を追加。提案していた6案のうち、案B（`literal_session_confirmed`のセッション跨ぎ拡張）は決定的反例（2026-07-22「kれでできる」）と案Cとの意味論衝突により削除、案D（recovery確定の先送り）は論理誤り＋premortemで「待つほど重複しやすくなる」逆効果が判明し要再設計、案E（recovery無効化）は実装記述の誤り（未送信VK消失という既知regressionの再現）を訂正した上で位置づけを変更。レビューで新規提案された案F（`grace_hold_verdict`の早期StaleConfirm確定をdeadlineまで先送り）＋案G（`veto_eligible`をidx==0/cold_seq新規のケースへ拡張、BUG-30安全機構の適用範囲是正）が、新定数・新観測チャネル・事後推測いずれも追加せず本incidentを防げる第一候補として収束。案C（回収を確認してから送る）は根本対策として有望だが、v1の具体化（HIDE確認）がBUG-75で棄却済みの自己汚染パターンに該当すると指摘され、具体的な確認チャネルは未解決のまま持ち越し | **ドラフト（Opus 2体round1完了、案F+Gが次段階の第一候補。未実装）** |
