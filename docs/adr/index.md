...
| [151](151-actuation-delegate-by-default-drift-scoped-to-ownership.md) | ADR-149「案D」の再検討として、delegate対象キーはawaseが一切actuateしない（belief追随のみ）方向を検証したが、「Appliedを詐称するとTsfNative唯一のON方向救済機構`apply_force_on_for_imm_broken`が構造的に永久停止する」というBlockerが見つかり保留 | 将来構想として保留（未実装）。ADR-149と同じBlockerを共有 |
| [152](152-keystroke-step-source-sink-pipeline.md) | 打鍵1回をsource（意図の発生源）→sink（実際の配送/actuation先）のパイプラインとして再構成する構想。ADR-151と隣接するが方向は独立 | 保留中構想（未実装） |
