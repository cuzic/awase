...
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。物理的に「かな系キー同士の入れ替え」は不成立（対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。r1「生SendInputでOSへ0xF2を送る」設計→Blocker3件発見・修正。r2「hook.rs側modifierゲート」で対処→改訂自体が全単射を破る新規Blockerを混入、アーキテクチャを「to=かなはOSへ二重配送せず既存のIME actuation経路に委ねる」設計へ転換（`transport.rs::plan`のF2分岐を役割代入由来では条件付きSuppress）。r3で安全性論拠を「送出VKがDBE系でないから安全」に訂正。r4でBUG-10食い逃げ救済経路の消失を発見、判定用フラグを新設。r5でそのフラグ自体のタイミング欠陥2件（ライブ値読み取りで機構全体が死ぬ／KeyUp再評価で毎打鍵オーファンKeyUpが出る）を発見・修正。r6でauto-repeat経路の新規Blocker（押しっぱなし中ずっと生の0xF2が漏れる）を発見し、決定2を「fresh press時点でSuppress/Allowを確定し、auto-repeat/KeyUpはその決定を踏襲する」ラッチ設計へ整理、actuation判定は既に`plan()`へ渡っている`shadow_toggled`を再利用する形に簡素化（delegate分岐は配線先の方向`Some(ShadowImeAction::TurnOn)`まで確認するよう訂正）。r7でfresh press検出手段自体が未定義（hook側`was_down`はメインスレッドから参照不能）と判明し、hook.rsが`is_fresh_press`を`RawKeyEvent`の明示ビットとして運ぶ設計へ訂正、ラッチの自己修復性を確保。「from=かな」方向（物理かなキー→安全な3キー）は引き続き生SendInputで届くため上流swallowガードのKeyUp消費対策を別途維持 | r7（Opus 2体敵対的レビュー7ラウンド、各ラウンドで新規混入した指摘を都度解消し収束中。r8レビュー未実施）。config/GUI(Phase C)は別ADRへ切り出し予定 |
