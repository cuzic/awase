...
| [190](190-msime-immcross-failure-fallback-idempotent-vk-ime-on.md) | Microsoft IMEでImmCrossが失敗したとき、非冪等なVK_KANJIトグルでなく冪等なVK_IME_ON/OFF(MsImeDirect)へフォールバックする(BUG-152) | **ドラフト v4(未実装)**。opus round1/2反映(撤去範囲の漏れ10件・数値誤り3件)+KanjiToggle撤去を決定に追加(round3確認待ち)。CI(a8/a9)検証済み、実機未 |
