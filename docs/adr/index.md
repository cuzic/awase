...
| [173](173-scope-solo-tap-ime-action-to-tsf-native.md) | `muhenkan_solo_tap_ime_action`/`henkan_solo_tap_ime_action`(ADR-153)を`AppImeProfile::TsfNative`限定にする。Windows Terminal + PowerShellで半角モード中の無変換/変換キーが「@」混入とIME ON固着を引き起こす(awase停止でも再現、GJI自身のTSFキー横取りが真因)ことへの対応 | 起草中、opus-adversarial-consult未実施 |
