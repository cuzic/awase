...
| [137](137-shift-katakana-dbe-mode-key-suppression-regression.md) | ユーザー報告「Shift+かなでカタカナにならない」を発端に調査。当初は「NICOLA小指シフト面（Face::Shift）の設計上そもそも無い機能」と誤診したが、ユーザーの「以前は動いていた」という指摘を受けOpusで`git log`を掘り下げ直し撤回: 真因候補は`runtime/transport.rs::PhysicalKeyDisposition::plan`がBUG-52修正（2026-08-05）で`VK_DBE_*`のKeyDownをShift押下有無を見ずに常時Suppressするようにしたリグレッション（BUG-116として起票）。当初案（`!event.modifier_snapshot.shift`を条件追加）はOpus 2体（architect/premortem役）4ラウンドの敵対的レビューで根拠不十分と判定され取り下げ: 「BUG-52はShiftなし、BUG-116はShiftあり」という前提自体がBUG-52実機ログで未検証（B-1）、`reinject()`が常時`wScan:0`で送るためAllowにしても実IMEに届かない可能性（B-2）、報告者アプリがStandard/ImmCrossなら無関係の可能性（B-3）、`modifier_snapshot.shift`のstuck実績（B-4）、scan付き注入のJISかな固着ハザード(BUG-15追補7/BUG-61)とその復旧手段(Alt+かな)をawase自身が常時swallowする(BUG-62)という安全上のBlocker(SB-1/SB-2)を発見。副産物として`executor::enqueue_reinject`の誤ったdocコメント（「PassThroughはCallNextHookExで直接届く」）も訂正。「確定した修正」ではなく「実機で検証する診断スパイク」に方針転換し、環境変数2軸(Allow/Scan)・3段安全ゲート(かなロック検出abort・belief上のIME ON判定・event.scan_code使用)・デシジョンツリー付きのスパイクを`diag/bug116-shift-katakana`ブランチ（develop非マージ）に実装済み | **前提未検証・実機検証スパイク作成済み（develop非マージ）。実機データ待ち** |
