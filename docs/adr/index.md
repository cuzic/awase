...
| [190](190-msime-immcross-failure-fallback-idempotent-vk-ime-on.md) | Microsoft IMEでImmCrossが失敗したとき、非冪等なVK_KANJIトグルでなく冪等なVK_IME_ON/OFF(MsImeDirect)へフォールバックする(BUG-152) | **ドラフト v3(未実装)**。opus round1反映+KanjiToggle撤去を決定に追加(round2確認待ち)。CI(a8/a9)検証済み、実機未 |
