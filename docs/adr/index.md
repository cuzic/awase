...
| [139](139-tracing-metrics-observability-migration.md) | ユーザー依頼「ログ/メトリクスをtracing/metricsクレートでRustベストプラクティスに準拠させたい、大胆かつ非連続的な変更も含めて」を受け起票。Opus 2体（architect/premortem役）の敵対的レビュー3ラウンドで収束。r1でarchitectが事実誤認6件を検出（実測1161箇所〈880超ではない〉、`app.log`ではなく`awase.log`、tracingは既にfacade3本のみ推移的依存でsubscriberスタックは全て新規、hook.rsの通常打鍵経路はログゼロで`hook_channel.rs`が既に同期I/O排除を構造的に実装済み等）。premortemがBlocker多数を検出: `architecture_guard.rs:1406`が`"log::warn!("`を構造マーカーに使い機械置換で確実にpanic（ADR-080不変条件6/BUG-33ガード）、`keymap.rs`の実行時`log::Level`分岐にtracing等価物なし、決定3の対象ファイルが`fix-requires-evidence.md`表と不一致（IME belief系が全滅）、決定4当初案（journalをtracing Layer化）は`Layer::enabled`にフィールド値が渡らず`with_app`再入で最重要イベントがdropされるため技術的に不成立。r2でarchitectが決定4に代案Option C（journal→tracingの一方向fan-out）を提案しB4/B5を解消、決定5(`metrics`crate)は`BugReportStateSnapshot`と重複するため不採用、決定5-1(awase-settingsパネル)は別プロセスでIPCが片方向のみのため実現不能と判明。r3でOption Cの設置場所を`push_journal_entry`から真の合流点`UnifiedJournal::absorb`へ訂正（premortem発見のB9、`JournalEntry::ImeEvent`が漏れる経路を防ぐ）、clippyのcognitive_complexity懸念は実測（同一構造で`log`/`tracing`とも同スコア29/15）で否認 | 採用（Opus 2体の敵対的レビュー3ラウンドで収束、Blockerゼロ）。実装未着手 |
| [140](140-ime-probe-actuation-quiet-window.md) | BUG-113の「二重actuation」とは独立に残置された、`kp_stage_idle_conv_check`のクロスプロセスprobe読み取りとGJI actuationの発行タイミング競合（真のレースではなく決定論的順序）を扱う。Explore 2体+Opus設計2体の相互批判で、既存フェンス（`conv_mutation_seq`等）は「issue自体を止める機構」を持たないため原理的に検出不能と判明。GJI actuationの発行経路（同期`ImeController::apply`/非同期`open_chain::fallback_write`/`tsf::send::send_eager_warmup_vk_pair`、他にも未監査の経路が残りうる）を実コード照合済みで記録。排他機構（Step 1）は排他窓の量が実測必須（`tuning-constants.md`）のため未着手とし、`win32.rs::send_input_safe`（`IME_KANJI_MARKER`判定）と`imm.rs::send_ime_control`（`now_timestamp_us()`基準の追加ログ、既存`send_health`用`current_tick_ms()`計測は不変）へのStep 0診断ログ追加のみを本ADRで採用 | 採用（Step 0のみ）。Step 1は実機実測待ちで未着手 |
| [141](141-physical-key-role-substitution.md) | ユーザー要望「変換/無変換/スペースキーを相互入れ替える秀Caps相当の機能（Alt系は対象外）」の基盤ADR。ADR-110（`key_remap`撤回）はCapsLock固有事情、ADR-130（`[[keymap]]`のtoへのIME制御VK許可を却下）は`SendInput`後段注入という実装方式固有の欠陥であり不可能性ではないと再整理。既存の`state/alt_impersonation.rs`のhook_callback最初期vk書き換え方式を一般化する方針。Opus 2体の敵対的レビューを3ラウンド実施し収束: r1でひらがな/`VK_KANA`のスコープ定義誤り（安全な3キーへ縮小、かな系は後続ADRへ）・`transport.rs::plan`等下流合流点未記載・latch解放漏れ4経路・hold-state設計がADR-110 r3の欠陥（BUG-100が「r2より悪化」と断じた方式）を再導入していた点を検出。r2で「from/toが親指キーvkと一致する組み合わせを全面禁止」という決定4が既定config（左=無変換/右=変換）で機能を丸ごと無効化する致命的欠陥と判明、両レビュアー独立に「ルール集合は3キー上の全単射（置換）」「Alt impersonationの出力には役割代入を適用しない」の2条件に置換。r3でその改訂作業自体が混入させたBlocker（決定1の判定条件`rewritten_vk==vk`が挿入点で恒真になる、決定7のKeyUp注入`reinject()`呼び出しがフックスレッドからの安全契約違反、決定8本文の編集事故による脱落）と他プロセス注入イベントへの`!is_injected`ガード未決定を検出・修正 | r3で収束（Opus 2体3ラウンドレビュー、Blockerゼロ）。実装はPhase B（本ADRの後続ADR）で着手 |
| [142](142-key-role-config-and-gui.md) | ADR-141（Phase A）を前提に、実際のTOML config形式・`awase-settings`設定GUI・バリデーション配置を決めるPhase B ADR。`[[key_role]]`という新規テーブル名（`[[keymaps]]`/`[[key_remap]]`との混同回避）でfrom/toペアのTOML表現を採用。Opus 2体の敵対的レビューを3ラウンド実施し収束: r1でr0のcore/platform分割案（バリデーションをcoreの全単射構造チェックとplatformのAltセンチネル不動点チェックに分割）が`from_name`のキー名別名解決がplatform専属であるため成立せずissue #99と同型の二重管理バグを再導入すると両エージェント独立に判明、全てplatform側（`state/key_role.rs`）に一本化。GUIは3要素上の全単射が6通りしかない事実を利用し6プリセット選択式にして構造的に無効な組み合わせを排除、グレーアウト判定は`resolve_thumb_key()`の戻り値vkから計算する（スロット位置からではない）。バリデーションエラーはconfig.tomlを一切書き換えず実行時無効化のみに留める（`validate()`での無効化は手書きルールを消失させるデータ損失になると判明したため）。r2/r3でAPI変更点・テスト実行可能性（Windows専用CI vs Linux純粋関数）の精査を反映。r4でテスト計画のためにADR-141決定2へ申し送った`event_eligible`パラメータ自体に、無条件早期returnがBUG-41と同型のstuck keyを再導入する欠陥を両エージェント独立に発見、`decide_alt_impersonation`の`engine_enabled`と同じ新規押下限定参照に修正して収束 | r5で収束（Opus 2体5ラウンドレビュー、Blockerゼロ）。実装未着手 |
| [143](143-kana-key-role-substitution.md) | ADR-141決定0が除外した「ひらがな/かな系キー」の役割代入を扱う後続ADR。物理的に「かな系キー同士の入れ替え」は不成立（対応する物理キーはscan 0x70の1個のみ）と判明し、スコープを「物理かなキー（かなスロット）とADR-141の安全な3キーとのクロスファミリー入れ替え」に再定義。設計はr1〜r10で10ラウンドのOpus敵対的レビューを経て収束（r9・r10は両エージェントともBlockerゼロ、残る指摘は記述レベルのみ）: 「to=かな」は生SendInputでOSへ送らず、`transport.rs::plan`のF2分岐を役割代入由来では条件付きSuppressし既存のIME actuation経路に委ねる（安全性論拠は「送出VKがDBE系でないから安全」）。動的条件（`kana_role_active`・`actuation_will_fire`）はfresh press時点でのみ評価し`plan()`呼び出し元のラッチへ確定、auto-repeat/KeyUpはそのラッチを踏襲する（fresh press自体は役割代入自身のhold-state`{KEY}_WAS_DOWN`/`SCAN_KANA_WAS_DOWN`から`role_substitution_fresh_press: Option<bool>`として`RawKeyEvent`経由で明示的に運ぶ——別の状態機械を借用すると情報源が二重化しクリア規律が食い違うと判明したため、役割代入自身のhold-stateへ一本化）。「from=かな」方向（物理かなキー→安全な3キー）は引き続き生SendInputで届くため上流swallowガードのKeyUp消費対策を別途維持。詳細な変遷（各ラウンドで発見・解消したBlocker/Major）はADR本文のステータス節・変更履歴を参照 | 収束（r11相当、Opus 2体敵対的レビュー10ラウンド+持ち越し解消の追加協議、両エージェント最終承認・Blocker/Major/Minor/Nit全ゼロ）。持ち越し3点はすべて解消済み: (1)実装順序はADR-141→ADR-142→本ADR、(2)ADR-141決定1/ADR-142決定B7の`{KEY}_WAS_DOWN`更新規則を「vk変換は無条件・hold-storeのみ`event_eligible`でゲート」に統一(両ADRを修正)、(3)かなスロットのconfig記号は`"VK_DBE_HIRAGANA"`のみ受理と確定(決定8-1〜8-4)。解消の過程で新たにBlocker相当1件(かなスロットの恒等はテーブル引きスキップであり正規VkCodeへの書き換えではない、さもないとかなスロット未設定ユーザーのBUG-52/ADR-137挙動が壊れる)・Major相当1件(injected KeyUpによるラッチ迂回、条件はconfirmed_targetでなくラッチの値そのものに訂正)を発見・修正。config/GUI(Phase C)はGUI文言のみ別ADRへ切り出し予定 |
| [144](144-gji-keymap-kana-alternative.md) | ADR-143の複雑さへのユーザーの疑問（「GJI前提の補助ツールで80%満たせたのでは」）を受け、Google日本語入力(Mozc)自身のキーマップ機構でかな系キー入れ替えを代替できないか調査した比較用ADR。Mozc本体`key_parser.cc`のソース直接確認により、`henkan`/`muhenkan`/`kana`/`katakana`/`eisu`/`hankaku`/`kanji`はすべて有効なキーマップトリガー名と判明——「かな系キーをトリガーに指定できるか未確認」という当初の主要な不確実性は解消。特にShift+代入先キー→カタカナ相当はIME ON状態でのコマンド発火のため`keymap.txt`に1行追加するだけで実現できる可能性が高い。残る未検証点は「DirectInput(IME OFF)状態で変換キーのKeyDownがMozcへ配送されるか」の1点のみ。MS-IME非対応・ADR-141の3キー汎用入れ替えは代替不可という制約により、awase全体の方針(IME非依存)としては不採用と判断 | 検討・比較用（実装しない前提の記録） |
| [145](145-scancode-map-kana-alternative.md) | 同じくADR-143との比較用に、既存Scancode Map機構(ADR-111/126)をかな系キー入れ替えに拡張できないか調査。実装コストは小さいが、エンジンON/OFF連動・Shift+代入先キー機能等の動的挙動は原理的に実現不可。一方、変換キーのスキャンコードをかなキー位置(0x70)に置換すれば、IME内部の「1回目ひらがな/2回目カタカナ」状態機械がそのまま自動的に働く可能性がある(未検証)。ADR-111背景が示すPowerToys由来の教訓(「IME関連キーのグローバルショートカットはフックより手前で扱う方が安全」)とADR-141/143のsentinel VK設計との緊張関係を明示的に検討。末尾にADR-141/142/143・144・145の3案比較表と結論を掲載 | 検討・比較用（実装しない前提の記録） |
| [146](146-gji-keymap-swap-tool.md) | ADR-144の唯一の未検証点（DirectInput状態での変換キー配送）をユーザーが実機確認したことを受け、ADR-143(hookベース)ではなくGJI自身のキーマップを書き換える軽量ツールを実装する方向で起票。既存`awase-gji-config`crate(config1.db読み取り基盤)を土台に、新規ロジックは「役割入れ替えテーブルの構築」と「keymap.txt出力」のみ。核心は「かなスロットの二重トークン性」(Kana/Hiragana=無シフト相当、Katakana=シフト相当という別トークン)を利用したShift正規化アルゴリズムで、これによりADR-143 decision7が大掛かりな実装で実現した「Shift+代入先キー→カタカナ」が変換規則から自動的に生まれる。出力はkeymap.txtのみでconfig1.db直接書き込みはしない(既存の恒久判断「config1.db書き込み編集は復活させない」を踏襲) | 提案中（実装前、敵対的レビュー未実施） |
