...
| [140](140-physical-key-role-substitution.md) | ユーザー要望「変換/無変換/スペース/ひらがなキーを相互入れ替える秀Caps相当の機能（Alt系は対象外）」の基盤ADR。ADR-110（`key_remap`撤回）はCapsLock位置キー固有のOSロック状態・IMEグローバルショートカット競合が理由であり対象4キーには非該当、ADR-130（`[[keymap]]`のtoへのIME制御VK許可を却下）は`SendInput`+`INJECTED_MARKER`による後段注入というADR-130が選んだ実装方式固有の欠陥であり「対象VKを送ること自体」の不可能性ではないと再整理。既存の`state/alt_impersonation.rs`が「hook_callback最初期でvkをローカル書き換えし、以後の全パイプライン（`classify_key`/`RawKeyEvent`構築）がそれだけを見る」「hook_callbackは常に握りつぶし、実出力は既存のRelay/`ReinjectKey`経路が担う」という第三の実装方式を既に本番稼働させていることを発見、これを一般化する方針。Alt限定の複雑さ（`GetAsyncKeyState`で直接読まれるOSモディファイア）は対象4キーに非該当と確認 | 提案中（r0、Opus敵対的レビュー未実施）。実装はPhase B（本ADRの後続ADR）で着手 |
