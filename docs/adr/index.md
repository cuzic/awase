...
| [128](128-uncorroborated-physical-ime-key-engine-lockout.md) | 不具合報告`01M1MMK8987NT5B2W73PCPZNZ1`（Windows Terminal+PowerShell、GJIで余分な「＠」出力）の根本原因調査から起票。journal/app_log突合により、物理CapsLock位置（scan=0x3A）から届いた`vk=0xF0`(`VK_DBE_ALPHANUMERIC`)1回を`kp_stage_shadow_ime_toggle`が即座に「物理IMEキーによるIME OFF」と確定しNICOLA変換エンジンをdeactivateする一方、実際のGJI変換状態(observed)はopenのままdrift補正が収束せず、不具合報告ボタン押下まで約29秒間エンジンOFFが続き、その間の物理キーがNICOLA変換を経由せず生のJIS配列文字（＠）のまま素通りしたと確定。BUG-15追補7（同じvk×scanの組み合わせがkbd106ドライバの副産物として実IMEの意図を伴わず発生しうるという既知の不安定性）とBUG-68（drift correctionの再武装cooldown）を踏まえ、(A)証拠強度の低いVK×scan組み合わせの即時belief確定を弱める、(B) observedが一貫して反対し続けたらdesired側を訂正する有界reconciliationを追加する、の2方向を論点として提示。ADR-121（no-op側の欠落）と対をなす、Send側が無期限に空振りするケースの欠落 | 設計検討中（ドラフトv1、レビュー前） |
