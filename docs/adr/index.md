...
| [190](190-msime-immcross-failure-fallback-idempotent-vk-ime-on.md) | Microsoft IMEでImmCrossが失敗したとき、非冪等なVK_KANJIトグルでなく冪等なVK_IME_ON/OFF(MsImeDirect)へフォールバックする(BUG-152) | **確定(実装に進んでよい)**。opus round1〜3で収束(Blocker無し)+KanjiToggle撤去を決定に追加。未実装・実機未。CI(a8/a9)検証済み、実機未 |
