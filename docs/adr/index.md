...
| [176](176-behavioral-calibration-of-ime-mode-key-shadow-overrides.md) | awase-settingsに専用較正UIを新設し、モードキーの実効果をユーザー協力の下で明示的に測定、gate_thumb_key_ime_actions出力を差し替えて静的分類を補完する。BUG-143の静的パースの限界を補完 | round1〜4で計17件のBlocker検出、v5で対応方針確定後、v6(較正専用ネイティブウィンドウ案)はopus round5で即日撤回、追加実機検証でegui本体ウィンドウ上でも観測可能と確定、v7でawase.exe本体が決定2と同じIPCで観測も兼務する設計に単純化、opus再レビュー待ち |
