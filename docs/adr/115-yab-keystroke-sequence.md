...
status: |-
  ...
  **決定8追補（2026-09-13）**: `keystroke_sequence`の既定値を`Off`→`On`に変更し、
  設定GUI（上級者向け設定タブ）にチェックボックスとして公開した。決定8が挙げた
  「既存のやまぶき派生ファイルに偶然含まれる`CV41`」は、レビュー時点では抽象的な
  懸念だったが、実際にはIssue #118報告者自身の実在レイアウトファイル
  （`CV41`/`CV58`/`CV43`/`CV56`/`CV4D`を33セル含む）そのものであり、この
  ユーザーにとってはOnが「意図しない暴発」ではなく本来求めていた動作だった。
  `CV`+16進数2桁・セル内`+`区切り・`@`+マクロ名はいずれも無関係な文脈で偶然
  一致するには十分特殊な文字列であるため、既定Onのリスクは実用上小さいと判断
  した。`layout/nicola_kakutei.yab`（句読点で確定、[[docs/adr/109-yab-cv4d-punctuation-auto-confirm.md]]参照）を選ぶだけで動くようにするための変更でもある。
  Offは、この構文の解釈自体を望まないユーザー向けの明示的オプトアウトとして残す。
related_adr:
  - "ADR-045"
  - "ADR-104"
  - "ADR-107"
