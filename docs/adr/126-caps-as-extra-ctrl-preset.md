...
**採用・実装済み（2026-09-03、r6。設計レビューはOpus 2体4ラウンドで収束
——r1: B1-B3、r2: B4-B5・M8-M15、r3: R1-R5、r4: 決定4項目5追加のみで収束確認。
Codex CLIによる実装後、Opus敵対的コードレビュー1ラウンドで、実装時に
一時追加された順序ヒューリスティックのBlocker（新規失敗モード2件）と、
実HKLMを叩く危険なテストのHigh指摘を受け、決定3に「原理的限界」として
明記の上で撤回・修正（r6）。Windows実機ソーク未実施）。** ADR-111 で採用した Caps(英数)⇔Left Ctrl
...
**原理的限界: Swap の2エントリと「CapsAsExtraCtrl + 値が偶然一致する
第三者エントリ」はレジストリのバイト列だけでは区別できない。** 第三者
ツールが `(0x1D→0x3A)`（Swap の逆方向エントリと完全に同じ値）を書いて
いる状態で CapsAsExtraCtrl を有効化すると、結果のエントリ集合
`{(0x3A→0x1D), (0x1D→0x3A)}` は真の Swap が書く集合と**集合として
完全に一致する**（`current_preset` の包含判定は `Vec` の順序を見ない
`contains` を使うため、物理的な並び順にも依存しない）。この状態を
`current_preset` に渡すと Swap 優先判定により Swap と検出され、無効化
すると第三者エントリごと消える。

これはコードのバグではなく、**「今どのプリセットが有効か」の唯一の
情報源をレジストリの内容そのものに限定した**（決定3冒頭、ADR-111決定3を
踏襲）設計の必然的な帰結であり、原理的に解消できない——「awase が最後に
書いたのはどちらか」を判別する追加の手がかり（レジストリの別の値、
設定ファイル上のマーカー等）を持たない限り、この2状態はビット単位で
同一である。

実装時にこの曖昧性への対処として、書き込み順序（`compute_new_entries`
が保持エントリを既存順で先に・`target` のエントリを末尾に置くという
内部規約）を手がかりに2状態を判別しようとするヒューリスティックが
一時追加されたが、Opus敵対的コードレビューで次の2つの実害が指摘され
撤回した:

1. 対象を無効化(`target=Off`)時のみに限定していたため、
   `target=CapsAsExtraCtrl`（曖昧な状態からさらに CapsAsExtraCtrl を
   選び直す遷移）には効かず、B1が別経路で再発する。
2. 順序ヒューリスティックの前提（レジストリ上の物理的な並びが awase の
   最終書き込み時の内部生成順と一致し続ける）は、第三者ツールが
   Scancode Map 全体を自分の並びで書き直す、値の間に別エントリが
   挟まる等で容易に崩れる。崩れると**真の Swap を無効化したときに
   逆方向エントリが消し残り**、再試行しても直らないという、対処前より
   悪い失敗モードを生む。

よって本 ADR は、この曖昧なケースを「`current_preset` の Swap優先判定に
従い、Swap として扱う（＝無効化時に一緒に消える）」という**単純で
予測可能な挙動**のまま受容する。第三者エントリが実際に消えるのは
「その値が Swap の逆方向エントリと完全に一致し、かつ awase のいずれかの
プリセットが有効化された」という狭いケースに限られ、それ以外（0x1D に
何か別の値をリマップしている、awase のプリセットを一度も有効化して
いない等）では第三者エントリは常に保護される。将来この曖昧性そのものを
解消したい場合は、レジストリ外にプリセットの由来を記録する別ADR相当の
設計が必要になる（却下した代替案参照）。
...
- **`ReadError` 時もラジオ操作を許可する（r2 で検討し撤回）**: 既存値が
  ...
  無効化する方針にした。
- **書き込み順序を手がかりに「真の Swap」と「CapsAsExtraCtrl + 値が偶然
  一致する第三者エントリ」を判別するヒューリスティック（実装時に一時
  追加、Opus敵対的コードレビューで検討し撤回）**: `compute_new_entries`
  が生成するエントリ列の物理的な並びを見て2状態を区別しようとしたが、
  (a) 対象を無効化時のみに限定していたため CapsAsExtraCtrl への切替
  時にはB1が別経路で再発する、(b) 並び順の前提は第三者ツールがレジストリ
  を書き直す等で容易に崩れ、崩れると真の Swap の無効化時に消し残りが
  発生し再試行しても直らない、という対処前より悪い2つの失敗モードを
  作ることが判明し撤回した。決定3の「原理的限界」として素直に受容する
  方針に置き換えた。
- **レジストリ外にプリセットの由来を記録するマーカー（設定ファイル等）
  を導入し、曖昧性そのものを解消する**: 決定3の原理的限界を技術的に
  解消する唯一の方法だが、Scancode Map 自体は OS レベルの機構で
  awase の設定ファイルとは独立して存在し続けるため、マーカーとの
  同期が別途壊れうる（例: awase をアンインストール後に手動で
  Scancode Map だけ残る、マーカーファイルだけ消える等）という新たな
  整合性問題を持ち込む。本 ADR のスコープでは「レジストリの内容のみを
  唯一の真実の情報源とする」という ADR-111 以来の単純さを優先し、
  この解消は将来必要になった時点で別 ADR として設計する。
...
2. **単体テストで必須カバーすべきケース**（Linux 上で `cargo test -p
   ...
   - **第三者が Left Ctrl(0x1D) を awase のプリセットと無関係な値に
     リマップしている場合、そのエントリが CapsAsExtraCtrl の有効化・
     無効化を通じて保持されること**（B1 の回帰テスト、現実的な多数派
     ケース）。あわせて、値が Swap の逆方向エントリと偶然一致する
     狭いケースでは第三者エントリごと消えること（決定3「原理的限界」の
     固定化テスト、バグではなく受容した既知の限界であることをコードで
     裏付ける）。
