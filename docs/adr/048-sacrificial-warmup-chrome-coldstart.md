...
[cold 検出]
   ...
   │   └── timeout → VK_IME_OFF→VK_IME_ON GJI 強制リセット → 再試行
   │
...
**原則**: Chrome 系アプリでは VK_IME_OFF（旧 F22）を ChromeProbe 後に送らない。
F2（VK_DBE_HIRAGANA）のみで GJI を活性化する。
TSF context リセット目的の VK_IME_OFF は必ず composition context を破壊する可能性がある。
...
VK_IME_OFF→VK_IME_ON シーケンスで GJI を強制的に Hiragana mode にリセットする。

このシーケンスには Hiragana 確認のための待機が必要なため、専用 FSM を設けた:

```
ImeModeFsm:
  Unknown → (VK_IME_ON sent) → WaitingHiraganaConfirm → (NameChange) → Hiragana

ChromeGjiReinitFsm:
  Idle → (timeout) → EmittedImeOffOn → (ImeModeFsm::Hiragana) → Complete
```

`ImeModeFsm` は VK_IME_ON/OFF 送信後の belief を更新し、
