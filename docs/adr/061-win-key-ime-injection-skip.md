...
VK_IME_ON / VK_IME_OFF / `VK_DBE_HIRAGANA` を `SendInput` で注入すると、
...
4. `SendInput(VK_IME_ON)` / `SendInput(VK_DBE_HIRAGANA)` が Win キー押下中に到達する
5. OS が Win+VK_IME_ON 等の未認識ショートカットとして処理し、Win↑ 時にスタートメニューが起動する
...
- `send_ime_mode_key` と `send_vk_dbe_hiragana_pair` の両エントリポイントをカバーしており、
  VK_IME_ON / VK_IME_OFF / `VK_DBE_HIRAGANA` の全注入パスが保護される
...
- ADR-057: GJI キーバインド F13/F14 → F21/F22 移行（廃止済み。現在は VK_IME_ON/OFF を使用）
