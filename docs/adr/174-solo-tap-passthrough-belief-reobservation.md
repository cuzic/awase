...
status: |-
  ...
  **2026-09-15追記（同日中の再検証セッション）**: 上記「charset軸
  デッドロック仮説の反証」は再検討が必要と判明した。前回のA/B実験は
  `SendInput`ベースの`VK_DBE_SBCSCHAR`注入だったが、`WM_IME_CONTROL`
  （`IMC_SETCONVERSIONMODE`）による直接書き込みでは実際にcharset
  (FULLSHAPE)を動かせることを確認した——前回の「反証」は「charsetが
  動かない」ことの証明ではなく「SendInput注入がGJIのcharset処理に
  届いていない」ことの証明だった可能性が高い（詳細はBUG-142.md
  「H1決定的検証セッション」節参照）。また、open軸は物理DBEキー押下
  のたびに正しく動く一方、charset軸は物理キー押下では一度も動かず
  `WM_IME_CONTROL`書き込みでのみ動く、という非対称性を実機で確認した
  ——これはcharset軸デッドロック仮説が想定する構造（open軸だけ動く
  経路があり、charset軸を動かす経路が構造的に無い）と整合する。
  ただし本ADRが対象とする「IME ON固着」自体はこの再検証セッション
  （2回）のいずれでも再現しなかったため、この非対称性と固着の因果は
  依然未確定。
related_adr:
  - "ADR-153"
  - "ADR-173"
...
### B1の実機確定（2026-09-15、6フェーズ実機検証の一部として）

上記の因果連鎖のうち「belief乖離→固着」自体が2026-09-15まで実機で
証明されていなかった（round1 Blocker B1）。同日、以下の**最小手順**
（タイピング一切不要）で確定した:

```
1. 半角モード（belief=false, 実GJIも初期状態はOFF）
2. 変換キーを1回押す（NICOLA親指キーとしては消費されずOSへ再注入
   passthrough。[shadow-toggle]ログなし、beliefは更新されない）
3. 半角/全角キー1回目 → 正常にON（[shadow-toggle] intent 昇格:
   action=TurnOn false→true、実送信 GjiDirect send 0x0016）
4. 半角/全角キー2回目（0.8秒後）→ 既に [shadow-toggle] intent 昇格:
   action=TurnOn **true→true**（同方向、変化なし）→
   [shadow-toggle] no-op: effective_open は既に true → apply-ime 見送り
   （以後、20回以上の連打すべてが同じ vk=0xF4（TurnOn方向）/no-opで固着）
5. 固着中にawase.exeをtaskkillすると、押下を続けたまま即座に正常な
   交互切替へ復帰した（[BUG-142](../known-bugs/BUG-142.md)参照）——
   OS/キーボードドライバ側の恒久的な破損ではなく、awaseのフックが
   生きている間だけ発生する現象であることも確定した。
```

**タイピングは不要**（当初の再現手順が要求していた「1文字入力」は
本質ではなく、単に変換キー単独タップ後の最初の`半角/全角`キー操作
だけで十分だった）。ステップ3の時点で、awaseの`belief`は「false→true」
という**正しい遷移**として記録されているが、これは**変換キーの
passthroughで既にGJIの実状態が動いていた可能性を無視した、誤った
起点からの遷移**である疑いが強い——ステップ3で awase が
`GjiDirect: send 0x0016`を実送信していることが、GJI側の内部状態
（無変換/変換キー自身の巡回ロジックが保持する状態、あるいはOS/
キーボードドライバが物理半角/全角キーの次回報告方向を決めるために
参照する状態）を追加で乱している可能性がある。
...
3. **観測結果のbeliefへの反映経路**: 既存の`ObservationSource::
   ...
   という同種の罠を繰り返さないこと）。
4. **なぜ「観測してbeliefを直すだけ」でBUG-142が解消するはずか**（ユーザー
   指摘、2026-09-15）: 単なるbelief衛生の問題ではない。実機ログでは、
   変換キーpassthrough直後の最初の物理半角/全角キー押下時に、awase
   自身がstale belief（false）に基づいて`GjiDirect: send 0x0016`を
   **実送信**していた。GJIが変換キーpassthroughの副作用で既に実際に
   ONへ遷移していたなら、この直後の再送信自体が「既にONのGJIへ追加で
   openコマンドを送る」という、通常のフローでは起こらない余分な
   actuationになる。再観測によってbeliefがこの1回目の物理キー押下前に
   correct（true）へ訂正されていれば、この余分なactuationは
   `[shadow-toggle] no-op`として送信されず、後続の物理半角/全角キーの
   OS側トグル方向報告が乱される契機（正確な機構は未特定、
   [BUG-142](../known-bugs/BUG-142.md)「実機検証」節参照）自体を
   未然に防げる可能性が高い。**目標は「beliefを正しくすること」自体
   ではなく「stale beliefに基づく誤ったactuationの発生を防ぐこと」**
   であり、settle時間はこの1回目の物理キー押下より十分先行して完了する
   必要がある（実機では変換キー押下から次の物理キー押下まで最短でも
   800ms前後の間隔があったため、数百ms程度のsettle時間なら十分間に
   合う）。
