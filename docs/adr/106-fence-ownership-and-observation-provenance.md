...
**訂正（PR 109 フォローアップ）**: 上記「恒久的に残す判断」は撤回した。
`observation_store.rs` 側の `update_focus_hwnd` ログ（旧値→新値付き）のみ恒久化し、
`focus_tracking.rs` 側の `[focus-hwnd-track]` は撤去した。前者は dispatch 時のみ発火
