...
制限。機能自体（複数ステップの `to`、TOML 手書き）は制限を受けない。

- **既存ルールの `to` にドロップダウンからステップを追加する手段が無い。**
  `awase-settings` の「ショートカット再割当て」タブで既存ルールにステップを
  増やせるのは ⌨ キャプチャ経由のみで、ドロップダウンから直接選ぶ「＋」相当
  の手段がない。さらに ⌨ キャプチャは `egui` が変換できるキーに限られ、
  PrintScreen や IME 関連キーはキャプチャ不可（ドロップダウン専用）——
  つまり **これらのキーを2ステップ目以降に置くことは GUI からはできず、
  `config.toml` の手書きが必要**。
- **⌨ キャプチャの挙動が新規ルールと既存ルールで非対称。** 新規ルール側の
  `to` キャプチャは値を「置換」するが、既存ルール側の `to` キャプチャは
  ステップを末尾に「追記」する（複数ステップを増やす唯一の手段がこれしか
  ないため）。同じ ⌨ ボタンが場所によって置換／追記に分かれており、UI上に
  その違いの明示はない。
- 両制限とも、GUI に「＋」ボタンを追加してステップ追加を明示的な操作にし、
  キャプチャを両側とも「置換」に統一すれば解消できる（既存ルール側は
  「＋」で空ステップを追加→ドロップダウンで選択、という流れにする）。
  今後の改善候補として記録するに留め、本 ADR の実装スコープには含めない。
