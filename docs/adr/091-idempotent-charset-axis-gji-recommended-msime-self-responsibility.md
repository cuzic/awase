...
**2026-09-02追記: §D3.2の専用Fnキー変換のうち、自動判定・設定支援ポップアップ・
config1.db書き込み(§4 Phase1 項目1・3・5)を全撤去した。** develop未マージの
実験的機能のまま撤去し忘れて出荷され、実機でユーザーが「GJIのキー設定は実際には
カスタムなのに、ポップアップ同意後に『カスタム以外』エラーが出る」という混乱に
遭遇したことが発端(詳細は[experiments.md エントリ19](../experiments.md)参照)。
`muhenkan_solo_tap_dedicated_fn_key`によるconfig.toml経由の手動設定と、
`nicola_fsm.rs`の専用Fnキー送出ロジック自体(§4 Phase1 項目2・4)は残っており、
上級者は引き続き手動で有効化できる。以降の§D3.1-D3.6・§4は、この撤去前の
設計判断の記録として残す。
