...
status: |-
  **採用（2026-09-26 所有者承認。opus round1〜4 で収束）。** 段階0（テスト）から実装に着手する。所有者の回答: 正規形は `VK_*` 名（推奨どおり）。未決事項1（起動時の告知）と4（ADR-199 T14 との関係）は実装時に決める。round4: 収束判定。確定前の3点（保存成功後の `base`、保存直前にディスクが無い/読めない場合、
