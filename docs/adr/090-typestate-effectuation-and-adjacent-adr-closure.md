...
**部分実装済み（2026-08-12）。** §3 の優先順位表のうち **C / B / F / E / A-1 / G
の 6 項を実装した**（それぞれ §2.C.6 / §2.B.5 / §2.F.6 / §2.E.5 / §2.A.6 /
§2.G.6 に実施記録と「ADR の記述と実コードが食い違っていた点」がある）。

**未実装は D（非同期チェーンの `caps` 再抽選化）と A-2（warrant の強制）の
2 項で、どちらも実機ソーク必須**である（§3 の順 6・7）。実装した 6 項は
**すべて挙動変更ゼロ**（A-1 はログと journal 用の起案が増えるだけ、
B は bit-identical、C / F は未配線コードの可視性縮小と削除、E / G は
ドキュメントと定数文字列）。

`tests/golden/ime_key_sequences.txt` を変更したのは **G のみ**（列のラベルと
doc 文字列だけで、戦略選択の期待値は 1 文字も変えていない）。
...
| A-5' | INV-47 は「`ActuationOrder::issue()` を通ってのみ起案される」 | `Actuation::request(` の本番呼び出しは `state/actuation_chain.rs` に**2 箇所残る**が、**2 箇所とも本番から到達不能な死んだコードである**（2026-08-12 の PR 最終レビューで判明。当初この行は「A-2 用」「同一 warrant からの再試行」とだけ書いており、**本番で実行されているかのように読めた**ので訂正した）。実測: (i) `ActuationOrder::into_actuation`（`:456`）の参照は**定義自体・`state/actuation_chain.rs` のモジュール doc・`tests/architecture_guard.rs` の説明文だけ**で、本番呼び出し元はゼロ。(ii) `DriftEpisode::next_attempt`（`:706`）も同様で、そもそも `DriftEpisode::new` の呼び出しが `actuation_chain.rs` の `#[cfg(test)] mod tests` にしか無く、**`DriftEpisode` 型ごと本番未配線**。A-1 後に本番で生きている `Requested → Warranted` 経路は **`into_actuation_shadow`（`ime_controller.rs:454` / `runtime/open_chain.rs:232`）の 1 本だけ**である | ガード `actuation_is_only_requested_through_actuation_order` を**件数ではなくファイル別内訳**で固定した（`[("src/state/actuation_chain.rs", 2)]`）。**`state/actuation_chain.rs` の外に出たら、それは warrant を持たない起案経路が復活したということ**である。**ただしこのガードが今固定しているのは「死んだコードが 2 箇所残っていること」であって、`Requested → Warranted` が型で守られていることではない**——`into_actuation_shadow` は warrant の有無に関わらず `Warranted` へ進める（授権が無ければ `Authorization::LegacyUnwarranted { would_have_blocked: true }` を載せるだけで**書き込みは止めない**）ので、**A-1 の時点では `Warranted` は「実 `OpenWarrant` がある」ことを意味しない**。ADR-089 §2.3 が意図した型による保証が効き始めるのは、**A-2 で入口ごとに `into_actuation_shadow` → `into_actuation` へ差し替え終えたとき**である（入口ごとに実機ソークが必須、§6 ステップ 7。`try_force_on_bootstrap` は最後）。**「INV-47 は守られている」と読まないこと。**A-1 で守られているのは「全 11 起案点が `ActuationOrder::issue()` = `issue_open_warrant()` を必ず通る」ところまでである |
...
- **INV-47（項 A）**: 実 actuation は `ActuationOrder::issue()` を通ってのみ
  ...
  **A-1 完了時点で成立しているのはここまで**である——`Warranted` に実
  `OpenWarrant` が伴うこと（ADR-089 §2.3 の型による保証）は、本番経路が
  `into_actuation_shadow` である限り**成立しない**。詳細は §2.A の A-5' を見ること。
...
2. **【解消（2026-08-12、§2.B.5 の B-2）: 分けなかった】** ~~**項 B の
   `ReadBackQuery::AnyFreshEvidence` は本当に `ConvergedReceipt` に
   載せるべきか。**~~ `Resolution` を 4 値にしたことで「収束したか」と
   「外界が動いたか」は `resolution()` の値として区別でき、型を分ける
   必要が無くなった。以下は起票時の記述: give-up 後の復旧判定は「収束したか」ではなく
