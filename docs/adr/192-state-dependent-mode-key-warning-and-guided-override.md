...
- **要確認**: (1)エンジンOFF中は親指FSMが動かない。この間はチョードの曖昧さが無いので、単独の押下でそのままactuateする経路が要る（強制ONは「エンジンOFFから起こす」用途が本命）。
  (2)確定までの遅延は同時打鍵しきい値ぶんになる。(3)`ime_toggle`の親指キー単体も対象に含める（`"toggle"`に正規化。awaseが書くのはbeliefに基づく開閉のトグルだけで、ADR-191の線引き〈開閉だけに作用するキーはawaseが書いてよい、ON/OFF以外の作用を持つひらがな・カタカナ等は観測して追随〉の内側）。ひらがな・カタカナ・英数のような入力モードキーは、親指キーに割り当てられていても正規化の対象にしない。
  (4)撤去ブランチ（ADR-191）で`*_solo_tap_ime_action`の配線が残っていること。
