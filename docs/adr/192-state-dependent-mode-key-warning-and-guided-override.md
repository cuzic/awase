...
### 決定3b: 強制ON/OFFキーを親指キー（無変換/変換）に設定した場合は「単独打鍵と判定されたとき」にactuateする

`keys.ime_on`/`keys.ime_off`に親指キー単体（`無変換`/`変換`）を書くユーザーがまれにいる。現状は設定検証が警告するだけ（`config.rs`のT-16）で、コンボ照合が親指FSMより前に走るため意図どおりには動かない。
- **新機構は作らない**。既存の`muhenkan_solo_tap_ime_action`/`henkan_solo_tap_ime_action`（ADR-153決定1、`resolve_pending_thumb_as_single`が「チョード不成立＝単独打鍵」と確定した時点でactuate）へ、設定読み込み時に正規化する:
  `keys.ime_on`の親指キー単体 → 当該キーの`*_solo_tap_ime_action = "on"`、`keys.ime_off`の親指キー単体 → `"off"`。チョード成立時は通常の親指シフトとして扱い、actuateしない。
- 冪等なON/OFFなので、ADR-191の「状態依存」問題は生じない。親指FSMがキーを消費するのでIME側の割り当て（状態依存のもの）も届かず、抑止も兼ねる。
- **訂正（opus round4、コードで確認: `engine.rs`の`suppress_ime_combos`、`config.rs`のT-16警告文）**: エンジンOFF中は、親指FSMが動かないので`keys.ime_on = ["無変換"]`は**既に動く**（強制ONの本命の場面）。動かないのは**エンジン活性中**で、
  同時打鍵の判定が優先されるため、親指キー単体の`keys.ime_on/off/toggle`が発火しない。本決定3bが埋めるのはこの穴（エンジン活性中の単独打鍵）だけである。
- **正規化先の制約（要確認、opusの指摘）**: `*_solo_tap_ime_action`は、入力中（composing）・`Passthrough`設定・専用Fnキー設定のときは無効になり、親指キーが未設定のVKでは何も起きない、とされる。
  この制約でユーザーの意図（強制ON/OFF）が満たせない場合は、正規化ではなく、`keys.ime_on/off/toggle`側に「親指キー単体は単独打鍵確定時に発火する」経路を足す形を再検討する。
  **実装前に、この制約を`resolve_pending_thumb_as_single`のコードで確かめる**（本決定は未検証の設計案。opusは「丸ごと削る」を推奨したが、ユーザーの要望〈親指キーを強制ON/OFFにしたい人がいる〉があるため、削らず前提を直して残す）。
- 遅延: 確定は同時打鍵しきい値ぶん遅れる。`ime_toggle`の親指キー単体も対象に含める（`"toggle"`。awaseが書くのはbeliefに基づく開閉のトグルだけで、ADR-191の線引きの内側）。入力モードキー（ひらがな・カタカナ・英数）は、親指キーに割り当てられていても対象にしない。
