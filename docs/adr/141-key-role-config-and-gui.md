# ADR-141: 物理キー役割代入のconfig形式・設定GUI（Phase B）
...
ADR-140（物理キー役割代入の基盤、Phase A、r3で収束済み）が確立した挿入点・
hold-state設計・全単射制約・Altセンチネル不動点制約を前提に、実際のTOML
config形式・`awase-settings`設定GUI・バリデーションの実装配置を決める。
ADR-140の未解決の疑問1〜3・決定4/6が明示的にPhase Bへ委ねた事項に、本ADR
で回答する。

対象VKはADR-140決定0と同じ「安全な3キー」（`VK_CONVERT`/`VK_NONCONVERT`/
`VK_SPACE`）。

**注記**: ADR-140決定6-3（`transport.rs::plan`への影響）が誤帰属で
あったことが本ADRのr1レビューで判明したため、**ADR-140本体にも訂正
注記を追加済み**（`docs/adr/140-physical-key-role-substitution.md`決定6
...
- **`[[keymaps]]`（ADR-114、`src/config.rs:589-604`の`KeymapRule`、実際の
  ...
  ADR-140決定1-2が「全単射制約の違反は個別ルールのskipでは修復できない、
...
を名指ししている。**r0はADR-140決定6-3を根拠にこの合流点への影響を主張
...
決定7（ADR-140）のKeyUp注入がBUG-100の再発ファミリーそのものである
...
- フィールド: `from: String`・`to: String`（単一VK名）。`[[keymaps]].to`が
  ...
  `to`は置換ではなくなり、ADR-140決定1-2の全単射制約・決定2の
...
- 明示されないキーは恒等（自分自身へ写る）として補完する（ADR-140決定1-2）
...
ADR-140未解決の疑問1（core/platformどちらに置くか）への回答。
...
**実行時に**無効化する（個別ルールのskipではない、ADR-140決定1-2/決定4）。
...
センチネルなら常に恒等写像のみ」という判定も誤りである——ADR-140決定4-1
...
計算すること**（r2レビューarchitect役r1-N2）: ADR-140決定4が「親指キーの
...
ADR-140未解決の疑問2への回答。**r0は「エラーにも警告にもしない設計も
...
代入後vkで照合されるため（ADR-140決定1）、このルールは**物理スペース
...
ADR-140決定6#6のPhase B注記への回答。**r0は「スペースキーの単独タップ
...
（ADR-140決定0が対象外としたVK_KANA等）が現実に含まれうる。この場合、
...
- **`golden_scenarios.rs`への追加は撤回する**（r1レビューB-6、architect役
  ...
  **r2→r3で「2層で責務を分ける」記述の型を訂正した**（r2レビュー
  ...
  ADR-140決定2のシグネチャ`-> (VkCode, Option<VkCode>)`のうち、hookが
...
- **`decide_role_substitution`の網羅テーブルテストを、値を区別する形に
  ...
  という規律を明記する。
- **ADR-140が要求した以下5種類のテストを、実際にLinuxで検証可能な
  ...
  **決定**: ADR-140決定1-1・1-3が定めるイベント単位の判定（Alt適用前後の
  比較・`!is_injected`ガード）を、`decide_role_substitution`（決定2）の
  パラメータへ吸収する形でADR-140決定2のシグネチャを拡張する（Phase A
  ...
  **r3→r4で`event_eligible`の参照タイミングを訂正した**（r3レビューで
  ...
  （ADR-140が「Down/Upが対で来ない注入は珍しくない」と明記した状況）と、
  ...
  **決定（訂正版）**: `event_eligible`は`decide_alt_impersonation`の
  ...
  1. **6変数のスロット混線防止テスト**は、`decide_role_substitution`を
     ...
     ——ADR-140決定2のN2（代入先でキーイングすると双方向スワップで
  ...
  **申し送り内容の追記**（r3レビューpremortem役S-3、r4レビューarchitect役
  Nit）: 上記のシグネチャ拡張はADR-140決定2だけでなく**決定1-1も
  ...
  **なぜ`!is_injected`の早期returnを無くしても実害が出ないか**（r4
  ...
  留まる——ADR-140決定7の「stuck-trueは危険だがstuck-falseはそうならない」
  という非対称と同じ性質である。Alt impersonation出力
  （`VK_NONCONVERT`/`VK_CONVERT`）が安全な3キーそのものと同じスロットを
  引く経路についても、ADR-140決定4の不動点制約によりそのスロットの
...
3. **解決済み**（r3、premortem役N-8指摘を受け先送りせず対応）: 本ADRが
   前提とするADR-140はr3までで収束しているが、r4（3回目の再確認）は
   未実施である。ADR-140決定6-3の誤帰属（`transport.rs::plan`ではなく
   `win32.rs:169`のみ——`keymap.rs`は無関係）は、本ADRのr1/r2レビュー中に
   ADR-140本体（`docs/adr/140-physical-key-role-substitution.md`決定6の
   3項目内）へ訂正注記を追加済み。次にADR-140のr4を実施する際、この
   訂正注記を本文へ正式に統合すること。
...
`#[cfg(test)]`、`keymap.rs`は対象外）。関連ADR: ADR-140（本ADRの
...
- r2（2026-09-06）: 同一レビュアーへの再確認（2ラウンド目）で、r1の
  ...
  B2/B3/B7を修正して反映。あわせてADR-140本体の決定6-3（誤帰属していた
...
- r3（2026-09-06）: 同一レビュアーへの再々確認（3ラウンド目）で、
  ...
  Linux実行不可能なhook.rs側配線の性質だった問題（ADR-140決定2への
...
- r4（2026-09-06）: 同一レビュアーへの再々々確認（4ラウンド目）で、
  ...
  （3-cycleプリセットでの期待値固定に差し替え）、申し送り先がADR-140
...
- r5（2026-09-06）: 同一レビュアーへの再々々々確認（5ラウンド目）で、
  ...
  の`is_injected`成分がデッドコードになる、ADR-140本体への申し送り
  ポインタが無いとADR-140単体を読む実装者が旧シグネチャのまま実装
  しかねない、申し送りブロックの位置がリスト構造を分断していた）を
  反映し、ADR-140本体（決定1）にも申し送りポインタを追加した。両
