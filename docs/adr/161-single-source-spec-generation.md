...
`docs/experiments.md`は1,043行・**25エントリ**（round1 M-5で訂正: 当初「17エントリ」と3箇所に
書いていたが実測は25件。`## エントリ`見出しは26件あるが、`docs/experiments.md`の28行目と707行目
が両方とも「エントリ17」を名乗る重複採番になっているため、実質25件。この重複採番自体が
`docs/experiments.md`側の未修正のバグとして残っている——「今後の議論」参照）。エントリ01は
...
8. `docs/experiments.md`のエントリ17重複採番バグ（round1 M-5で発見、28行目と707行目が両方
   「エントリ17」を名乗っている）を、本ADRとは別に修正する。
