...
**Phase 2（未実装）は2026-09-07に実機確認を実施**——`StyleList\Custom`
のパス実在・6プリセット構成・バイナリ形式・「IMEオン/オフ」トグル
割り当てのコード（`CE`/`CD`）・重複行による非対称な実効挙動を確認した
（詳細は後述「Phase 2 実機確認」節）。ただしPhase 2自体はまだ実装対象外
のまま。
