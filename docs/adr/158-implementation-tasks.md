...
### TG1: C1〜C3それぞれの判断材料を収集する（部分完了、2026-09-09）
...
を満たすこと、TB0、TB1。**2026-09-09時点でこの基準は未達成のまま**（TB0・TB1は完了済みだが、
ADR-159段階2の実装＝TF2は「既存機構で充足」という当初の完了報告が誤りと判明し実質未着手の
まま、opus code review S5参照）。規約の**発効**はこの依存が満たされてから行う。
...
**依存**: ADR-159の記録・再生基盤が能力ベースの基準（TH1と同基準、2026-09-09時点で未達成）を
満たすこと、TH3（E3の定例サイクルで対象を選定する運用を想定、TH3自体は完了済み）。
