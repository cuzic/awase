...
- **レート制限・サイズ上限の具体値**: 20件/IP/日、ボディ512KiB、説明欄
  4,000文字、ログ抜粋（journal/awase.log それぞれ）256KiB→**200KiBに
  引き下げ**（クライアント側でUTF-8境界を壊さず切り詰め。引き下げの経緯は
  「実運用での調整」節参照）。
...
- **添付するログの範囲**: 新規の範囲指定は設けず、既存の journal ダンプ
  ...
  使い、クライアント側で journal/awase.log それぞれを個別に切り詰める
  方針にした（「合計」ではなく「各々」の上限である点に注意。両方フル
  添付すると256KiB×2=512KiB=`MAX_BODY_BYTES`に達し他フィールド分だけ
  確実に超過する構造だったことが後日判明し、200KiBに引き下げた。
  「実運用での調整」節参照）。
...
`log_excerpt` の上限 `LOG_EXCERPT_MAX_BYTES`（200KiB、旧256KiB）や全体の
...
- ~~レート制限・サイズ上限の具体的な閾値~~ → 20件/IP/日・512KiB・
  説明4,000文字・ログ（journal/awase.log 各々）256KiB で実装済み。
  実際に実運用で調整が必要になり、200KiBに引き下げた
  （「実運用での調整」節参照）。
...
Windows 実機で「不具合を報告」を実際に使ったユーザーから3件の問題報告が
あり、以下を修正した（詳細は各コミット参照）。

1. **説明欄への入力遅延**: キー入力のたびにペイロード全体（最大~500KB）を
   同期的に JSON 再構築しており、体感できる遅延があった。300ms のデバウンス
   を導入。
2. **送信ボタンが見えない**: 送信ボタンが通常フローの末尾にあり、ウィンドウを
   広げないと見えなかった。`egui::TopBottomPanel::bottom` で画面下部に固定。
3. **送信サイズ上限に毎回引っかかる**: `log_excerpt`（journal/awase.log）の
   上限 `LOG_EXCERPT_MAX_BYTES` は決定9で256KiBと決めたが、これは
   journal・awase.log **それぞれ**に適用される上限であり「合計256KiB」では
   なかった（決定9の記述が誤解を招く表現だった）。両方フル添付すると
   256KiB×2=512KiB=`MAX_BODY_BYTES` に達し、他のフィールド（内部状態
   スナップショット・設定ファイル・配列ファイル等）のぶんだけ確実に超過する
   構造で、「送信のたびに必ず自動切り詰めが発生する」状態になっていた。
   `LOG_EXCERPT_MAX_BYTES` を200KiBに引き下げ、通常ケースでは追加の縮小が
   発生しないようにした（200KiB×2=400KiBとして他フィールド用に実測
   ~102KiBのマージンを残す。テストで固定済み）。

さらに、実機で送信自体が
`WinHttpSendRequest: パラメーターが間違っています。(0x80070057)` で
失敗する不具合も見つかった。`windows` crate の `WinHttpSendRequest`
バインディングはヘッダー引数 `Option<&[u16]>` の `slice.len()` をそのまま
`dwHeadersLength`（文字数）として WinHTTP API に渡すが、実装はヘッダー
文字列を NUL 終端込みで `collect()` していたため実際の文字数より1多い
長さが渡され、余分な NUL 文字がヘッダーの一部として解釈されていた。
NUL 終端を含めないよう修正した。
