...
summary: |-
  ...
  **設計の骨子**: (A) 「予測（本ADRのスコープ）」と「actuation可否（ADR-189の固定セット+ユーザー明示config、本ADRの範囲外）」は
  ...
  実時間）、(3)永続化（表全体を別ファイルに分離、スキーマ版つき）、(4)実行時読込（予測にのみ使う、actuationには使わない。読めない
  窓向けの受け入れ基準〈縮退率の閾値+同梱表とのセル突き合わせ〉と同梱表への退避を含む）、(5)隠れ状態を最小Mealy機械として持つ、
  (6)ADR-176ウィザードからの起動統合（子プロセス起動+標準出力での進捗）、(7)安全対策、(8)陳腐化検出（全体フィンガープリント+
  スキーマ版）。ADR-194（シミュレーション・リプレイのハーネス、破棄）とADR-192（状態依存キーの警告UI、検出ロジックは段階0で
  再利用）は範囲外（参照のみ）。
status: |-
