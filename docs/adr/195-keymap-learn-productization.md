...
summary: |-
  ...
  **設計の骨子**: (A) 「予測（本ADRのスコープ）」と「actuation可否（ADR-189の固定セット+ユーザー明示config、本ADRの範囲外）」は
  ...
  当てはまらない）、動的なIPC・begin/endのライフサイクル・keepaliveは不要にした。
  ...
  再利用）は範囲外（参照のみ）。
status: |-
  **草案 rev6（2026-09-22、opus-adversarial-consult round5まで収束。Blocker無し、round6は文言確認待ち）。**
  実装はまだ無い（`awase-keymap-learn`クレート自体は`feat/awase-calibration`ブランチにdevelop未マージで存在するが、巡回プランナと
  シミュレータのみで、本ADRが定める製品への組み込みは未着手）。改訂の経緯（round1〜5の各指摘と対応）はPR #244の議論・コミット
  メッセージに残る。
related_adr:
  - "ADR-019"
  - "ADR-090"
...
- **`awase.exe`との共存（A'の確保）——採用: `disable_apps`相当のコード内定数照合（round5 N-a）**: 学習プロセスの窓へ向かう
  ...
  - **BUG-14の`injected`ガードとの関係**: この策では学習プロセスの注入キーは`awase.exe`の同一LLフックを通る（プロセス
    ...
    一切のactuationを起こさないこと」をA'の実測項目に含める**。
...
1. **N-2（round4 Blocker）採用条件を「学習プロセスの検証フローに組み込む」は実行不能——2段階に分ける**:
   ...
   - **(ii) ユーザー環境で使える代理指標——閾値と行動を明記（round5 M-c）**: 段階2の独立ランダムウォークで計算できる
     ...
     次の2つを閾値・行動つきで追加する:
     1. **縮退率の閾値**: 縮退率が暫定`20%`を超えたら採用しない（M-b導入の代わりに`use_learned_keymap_table`を自動で
        `false`にする）。20%は実機プロトタイプが無い時点の暫定値で、根拠は段階1の実機プロトタイプで更新する。
     2. **同梱表とのセル単位の突き合わせ（同梱表がある構成のみ）**: 検出したキーマップ構成がATOK/GJI+MS-IMEプリセット/
        Microsoft IME本体のいずれかと一致する場合、学習表と同梱表を**セル単位で突き合わせ**、不一致セル数が暫定閾値
        （例: 5%）を超えたら採用しない。同じキーマップ構成なら本来一致するはずのため、この比較は追加の実機測定を要さない
        （両方のデータが既に手元にある）。この比較はawase-windows側の段階4読込コードが行う（同じクレート内なので
        `key_effect_table`の可視性は`pub(crate)`があれば足り、学習プロセス側から読む設計ではない——m-cの結論を
        この用途に限って更新する）。段階6のUI方針（「同梱表と同じ構成なら学習を勧めない」）と対になる。
...
- **進捗は子プロセスの標準出力で運ぶ**: awase-settingsは学習プロセスの標準出力（進捗〈現在何セル目/推定残り時間〉と成否だけ、
  ...
  進捗表示にも新設のメッセージ型を足す理由が無い。学習が完了すると段階3の永続化ファイルが更新され、段階4の実行時読込
  （次回のfsスタンプ再チェック時、または子プロセス終了をトリガにした即時再読込）で反映される——「適用」という別のユーザー
  操作は要らない。
