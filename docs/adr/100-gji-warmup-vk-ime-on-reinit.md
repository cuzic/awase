...
**決定2（`VK_IME_ON` 単発への置換）は2026-08-22、実機検証を経てユーザー判断により実装済み。決定1・決定3（提案1のトグル化・提案2のretry追加）は却下のまま。決定2の群A/群Cとの厳密な比較・BUG-69依存の群C検討は今後の課題として残る。決定3の案L（journal記録）は2026-08-23、BUG-74（ADR-100が予告していた「give-upによる文字消失の再報告」そのもの）への対応として実装済み——実装着手時に一度、提案2（retry）そのものを再発明しかけたが、コミット前に本ADRの却下記録を確認して破棄し、案Lを実装した経緯がBUG-74に記録されている。** 設計は premortem レビュー ラウンド1・ラウンド2（評決「N1〜N3 を反映すれば実装着手可、ラウンド3 の全面レビューは不要」）を経ている。ADR-098 決定3-c（「GJI の eager warmup キーを `VK_DBE_HIRAGANA` から `VK_IME_ON` へ置き換えられないか」、本 ADR では決定しないとして先送りされた宿題）と、[docs/experiments.md](../experiments.md) エントリ16（同件の事前登録）を正式に引き取り、ユーザー発案の2提案について結論を出す。
