...
**設計第3稿（premortem レビュー ラウンド1・ラウンド2 反映済み）。ラウンド2 の評決は「N1〜N3 を反映すれば実装着手可、ラウンド3 の全面レビューは不要（差分確認で足りる）」。実装 0 行。** ADR-098 決定3-c（「GJI の eager warmup キーを `VK_DBE_HIRAGANA` から `VK_IME_ON` へ置き換えられないか」、本 ADR では決定しないとして先送りされた宿題）と、[docs/experiments.md](../experiments.md) エントリ16（同件の事前登録）を正式に引き取り、ユーザー発案の2提案について結論を出す。
...
本 ADR の初稿・改訂2版の調査は Linux 上のコード読解と既存の実機ログ記録（`docs/known-bugs.md`）に基づいていた。**2026-08-22、決定2 群C（eager warmup 完全無効化）についてのみ実機測定を実施した（F15・F16、Windows Terminal / dragonflyg4）。** 第1回は診断ゲートの配置による交絡で無効、第2回（交絡解消後）は3シナリオそれぞれで最低1回の成功例を確認したが、群A・群Bとの比較を含む決定2の合格基準には遠く届いていない。**Opus advisor によるレビューで、この結果を撤去の正式決定へ格上げするのは時期尚早であり、むしろ BUG-69（ADR-098）F1/F2 の未参照という別の穴が優先度として先に立つと指摘された（P6 参照）。** 決定4 の残り項目・群A/B の実験は未実施のまま。
...
### 決定2: 案A（`VK_IME_ON` 単発への置換）を**実験として登録し、実機データが揃うまで実装しない**

ADR-098 決定3-c と `docs/experiments.md` エントリ16 を、より具体的な測定計画へ格上げする。

**実施状況（2026-08-22 追記、F15・F16）**: 下記の群A/B/C比較のうち**群C（eager warmup 完全無効化）のみ**を実機（dragonflyg4、Windows Terminal）で2回測定した。第1回は診断ゲートの位置による交絡で無効（F15）。第2回（交絡解消後、F16）は3つの狙ったシナリオ（IME OFF からのフォーカス復帰・63秒放置・高速連続打鍵）でそれぞれ最低1回、クリーンな成功例（`giving up`/literal 化なし）を確認した。ただし各シナリオ1〜4回に過ぎず、下表が要求する「各シナリオ5試行以上・3群比較」には遠く届いていない。**群A・群Bとの比較は未実施のまま**——群Bなしに「`VK_IME_ON` へ置換してよい」とは言えず、群Aとの頻度比較なしに「eager warmup は不要」とも言えない。
...
**P6（F15/F16 実機測定後に追加）: eager warmup 撤去を正式決定した3ヶ月後、TsfNative+GJI アプリへフォーカスを戻した最初の数文字が直接入力（英字）で出る。**
BUG-69（ADR-098）F1/F2/F3 は「TsfNative+GJI のフォーカス復帰時、実際に機能する IME actuation は eager warmup だけであり、他の2機構（force-on ブロック・`mirror_applied_open` 経由の訂正）は別のバグで到達不能」と結論している。**ADR-100 はこの結論をこれまで一度も参照していなかった。** F16 の実機テストは Windows Terminal に対して群Cを試したが、3シナリオとも「実IMEがONのまま」推移した可能性が高く、「実IMEがOFFの状態でフォーカスが戻る」（外部IMEトグル直後、Ctrl+無変換の救済直後等）という、BUG-69 が指摘する本当に危険な条件を明示的には踏めていない（シナリオ1は`ime_on=false(shadow)`を経由したが、`SetOpenTrue`自体は正常に発火しており、eager warmupが唯一のactuationとして機能しなければならない場面を必ずしも意味しない）。eager warmup を完全に撤去し、かつ BUG-69 F1/F2 を未修正のまま放置すると、フォーカス復帰時の IME actuation が構造的に1つも残らなくなる。**対策**: eager warmup の撤去を正式決定する前提として、BUG-69 F1/F2 の修正（`mirror_applied_open` の TsfNative スキップ・到達不能な force-on ブロックの整理）が先に入っていることを確認する。F16 の追加測定では「実IMEをOFFにした状態を作ってからフォーカスを戻す」シナリオを明示的に増やすこと。
