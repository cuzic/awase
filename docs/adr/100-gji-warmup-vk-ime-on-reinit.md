...
**実施状況（2026-08-22 追記、F15〜F18）**: 下記の群A/B/C比較のうち**群C（eager warmup 完全無効化、F15・F16）と群B（`VK_IME_ON` 単発、F17・F18）**を実機（dragonflyg4、Windows Terminal）で測定した。群Cは第1回が診断ゲートの位置による交絡で無効（F15）、第2回（交絡解消後、F16）で3シナリオそれぞれ最低1回のクリーンな成功例を確認。群Bは候補1の形態（scan実値+`TSF_MARKER`、F17で確定）で2ラウンド実施し（F18）、`giving up`/literal 化は一件も無かった。**いずれも「有望」だが、下表が要求する「各条件5試行以上・3群を同一セッションで比較」には遠く届いていない。群A（現行F2）を基準にした直接比較は一度も行っていない**——群B・群Cそれぞれ単独で「壊れなかった」ことは分かったが、群Aと比べて「同等」「より良い」とまでは言えない。
