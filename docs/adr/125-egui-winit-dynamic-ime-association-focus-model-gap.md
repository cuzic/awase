...
**調査中・設計未確定。実機（Windows）での裏取り前。** ユーザー報告（2026-09-03）を
契機に、`winit`/`egui-winit` の実ソース（`~/.cargo/registry` にキャッシュされた
`winit-0.30.13`/`egui-winit-0.31.1`）を読解して技術的な機構は特定したが、
この機構が実際に報告された症状の原因であることは実機ログでまだ確認していない。
**このため本 ADR は実装を決定しない。** 次のアクション（「次のアクション」節）を
実行し、実機データで裏取りしてから改めて実装方針を確定する。
...
## 調査で判明した技術的事実（ソースコード読解、実機ログでの確認前）
...
**本 ADR の時点では実装方針を確定しない。** 選択肢 C（HIMC 変化検知による
汎用フォーカス相当イベント）を最有力候補として残しつつ、次のアクションを
実行し、実際に何が起きているかを実機データで確認してから実装 ADR を
起票し直す。

### 次のアクション

1. 実機（Windows）で `RUST_LOG=debug` を有効にした状態で、不具合報告画面・
   設定画面の説明欄に症状（重さ・かな混入）を再現させる。
2. `journal`/`awase.log` を取得する（送信フォーム自体が対象の画面のため、
   アプリの送信機能ではなく手動でのファイルコピーを想定——保存先は
   `crates/awase-windows/src/journal.rs` 起点の既定パスを確認する）。
3. 取得したログを `docs/journal-replay-guide.md` のリプレイ基盤で解析し、
   「未確定な点」1〜3（`allow_ime` 遷移の有無・頻度、重さの実体、かな混入の
   実体）を裏取りする。
4. 可能であれば、デバッグビルドに一時的なロギングを追加し、
   `ImmAssociateContextEx`/`ImmGetContext` が返す HIMC の変化を実機で直接
   観測する（`egui-winit`/`winit` はソース改変せずログを追加できないため、
   awase 側から `ImmGetContext` を定点観測する形になる）。
5. 上記で機構が確認できた場合、選択肢 B/C を実測に基づいて再評価し、正式な
   実装 ADR を書く。確認できなかった場合（例えば真因が `bug_report.rs`
   自体の別経路だった場合）、本 ADR はクローズし、判明した実際の原因に
   応じた別の ADR/直接修正に切り替える。
