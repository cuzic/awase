...
**Phase C 実施記録に対する Opus レビュー（2026-08-12）で 3 点を訂正した**——
§9-21（「同期 ImmCross 経路は到達しない」は誤り。`try_force_on_bootstrap`
から到達する。ただし Phase C 以前から同挙動で新規の回帰ではない）、
§9-22（`verify_gen_only` の世代照合は構造的に恒真で実効的な検出力は無い）、
§9-17 冒頭（旧 17-a は実機ソークではなく通常 CI の `windows-build` ジョブで
検証される前提条件）。**いずれもプロダクションコードの修正は不要**で、
誤っていたのは ADR とコード doc の記述だけである。
...
- **`set_ime_romaji_mode()` の同期 IMC write**（`ime_controller.rs` の
  ...
  **ただし是正できたのは「宛先を write 関数が自己決定しない」までで、
  世代照合は恒真であり同一性の検証にはなっていない**（§9-22）。
...
INV-14 の達成度ではない。捕獲点を一本化したからこそ「ライブクエリ版を
再実装しないこと」を守り続ける必要がある（Phase A の教訓「型を書いたから
削除してよい、ではない」と同じ理由）。なお **Phase C で達成できた INV-14 は
「宛先を write 関数が自己決定しない」までであり、「起案時と実行時の
ターゲット同一性を検証する」までではない**（世代照合は恒真、§9-22）。
その意味でもテキスト検査を落とす段階には無い。Phase C は同じ役割のガードを 1 件
...
19. **【訂正】`VerifiedTarget::FocusImplicit` は「INV-14 の未達」ではない。**
    ...
    **潰せない**。VK 送信機構（`GjiDirect` / `MsImeDirect` / `KanjiToggle`）の
    ...
    `ActuationTarget` 化した（**ただし捕獲点の一本化までで、世代照合は
    恒真である**。§9-22）。**残る `FocusImplicit` の「本当の未移行分」は
