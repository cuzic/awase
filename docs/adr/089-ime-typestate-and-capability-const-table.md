...
**着手条件**: **ADR-088 トラック D（実機 SendInput 検証）の復旧。**
Phase C は実際に送るキーと順序を変えうるため、実機で検証できない状態で
入れてはならない（`docs/experiments.md` エントリ01 の 5 日間 6 回反転は、
まさにキー選択を実機検証なしに動かした結果である）。
...
5. **Phase C の着手条件（ADR-088 トラック D の復旧）が満たされる見込みが無い。**
   ADR-088 §7 のとおり、合成キー入力が原因不明で無効化される問題は未解決である。
   **Phase C が永久に着手されない可能性を前提に、Phase B までで完結する形に
   なっているか**は §6 で「なっている」と書いたが、実装してみないと分からない
   （`ime_controller.rs` の 2 経路一本化が Phase B で本当に完結するか）。
