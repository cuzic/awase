...
id: ADR-160
title: |-
  非スコープを決定する会議体を持つ（C1: IME一本化／C2: アプリホワイトリスト化／C3: conv-mode追跡全廃）
summary: |-
  ADR-158採用Cの子ADR。C1〜C3に判断材料と期限(ADR-162のE3連動)を追記し無期限確認待ちを解消。2026-09-09、実機スパイクでC3の予備実測(probe 2192件中conv_mode乖離0件)を追加、ADR-161の「宣言の強制とSSOT化」原則の適用可能性(判断材料・期限テーブルの構造化)を将来検討として記録(未実装)
status: |-
  起票。TJ3(単体レビュー)実施済み・round4反映済み。実施可否はユーザー確認待ちのまま未確定(2026-12-31バックストップ設定済み)
related_adr:
  - "ADR-095"
  - "ADR-136"
  - "ADR-158"
...
**C1（GJI一本化）— 収集完了（2026-09-10）**: `wrangler` OAuth経由でR2バケット
`awase-report-bucket`の全オブジェクト（50件、2026-08-23T16:16:54Z〜2026-09-09T01:28:24Z、
約2週間分。R2 lifecycle ruleにより90日で自動削除されるため、これが観測可能な全期間）を
ダウンロードし、各報告の`payload.ime_kind`/`payload.ime_product_name`を集計した
（`docs/bug-reports-triage.md`台帳ではなく、raw JSONを直接集計——台帳には`ime_kind`列が
無いため）。結果: **Gji（Google日本語入力）37件（74%）／MsIme（Microsoft IME）13件
（26%）**。MS-IMEユーザーは少数だが無視できない規模で実在する（約4件に1件）。この母集団は
「不具合を報告した」ユーザーに限られる（ADR-095のオプトイン機能）ため、全ユーザーに占める
MS-IME利用率の代理指標としては上振れ・下振れどちらの方向にもバイアスしうる点に注意
（不具合報告への到達率がIME種別で異なる可能性を排除できない）。**判断材料としての解釈**:
「MS-IMEユーザーの実在規模」という問いに対しては、実装労力を割いて対応を続けるに足る
規模（4件に1件）が確認された、というのがこの集計が示す事実。C1（GJI一本化）を実施する
実害（MS-IMEユーザーを切る）は、この26%という数字に照らして評価する必要がある。
