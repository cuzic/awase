...
**申し送り（2026-09-06、ADR-146決定10より）**: 本ADR（hookでのvk書き換え
による物理キー役割入れ替え）とは別レイヤーで、GJI(Mozc)自身のキーマップ
を書き換えることでGJI専用に同種の入れ替えを実現する軽量ツール
（[ADR-146](146-gji-keymap-swap-tool.md)）が並行して起票されている。
本ADRを実装に着手する際は、両者の併存関係（両方設定された場合の合成
挙動、設定GUIでの表示の重複）を必ず再検討すること。ADR-146起票時点
（2026-09-06）では本ADRは未実装のため実害は無い。
