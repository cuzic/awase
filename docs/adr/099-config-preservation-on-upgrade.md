...
- Windows の `rename`（`MoveFileEx(MOVEFILE_REPLACE_EXISTING)` 相当）は
  ...
  短いリトライ（50ms×5 = 250ms）で緩和し、リトライ尽きた場合は一時
...
- **SF-1**: `save()` は `crates/awase-windows/src/tray.rs:823` の
  ...
  スレッド（`run_message_loop`）上で実行される。リトライ最大250msの
  `thread::sleep` はこの間エンジンスレッドをブロックし、打鍵がバースト
  出力される可能性がある。`hook.rs` が全キーを飲み込みエンジンスレッドが
  再注入する設計のため取りこぼしは起きないが、体感遅延が生じうる。
  `save()` はユーザーが明示的に「適用」を押した時とトレイの自動起動
  切替時のみ呼ばれる低頻度操作であり、リトライは rename 失敗という
  稀なケースにのみ発動するため許容するが、実機で体感遅延が問題になれば
  リトライ回数を減らす（既知の限界に記載）。
...
- Windows 実機での ZIP 版アップグレード手順（決定1・2適用後）の実機
  確認は未実施。
- 決定3のリトライ幅（50ms×5）は実測に基づかない初期値。
  [tuning-constants](../../.claude/rules/tuning-constants.md) の対象
  タイマーではない（`tuning.rs` 外）が、実機で AV/OneDrive 等との競合が
  観測された場合は値を見直す。リトライ中は `save()` を呼んだスレッドが
  最大250msブロックされる（round2 指摘 SF-1）。`tray.rs::save_auto_start_config`
  経由ではエンジンスレッドがこの間ブロックされるが、打鍵は
  `hook.rs` が全て飲み込み後で再注入する設計のため取りこぼしは無く、
  体感上は入力のバースト遅延にとどまる想定（実機未検証）。
  `*.toml.tmp.<pid>` はプロセス間の一時ファイル名衝突は防ぐが、同一
  プロセス内の複数スレッドから同時に `save()` が呼ばれる場合の排他は
  保証しない（round2 指摘 SF-6）。`save()` は単一スレッドから呼ぶ設計を
  前提とする。
