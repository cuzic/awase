...
再現条件は BUG-25 追補5 を参照。M4（Shift 押下中）はこの発見のため未実施
のまま保留し、**決定2 の実装には KeyUp 欠落を織り込んだ追加のセーフガード
（settle 待ち・連続発火の最小間隔）が必要**という形で下記の決定2/決定9に
反映した。
...
| # | マーカ | awase | Shift | 期待/仮説 |
|---|---|---|---|---|
| M1 | none | 停止 | 解放 | 成功（＝追補4 経路6 の再確認、基準線） |
| M2 | none | 起動 | 解放 | 失敗（＝追補4 経路5 の再確認、原因A の再現） |
| M3 | `IME_KANJI_MARKER` | 起動 | 解放 | **成功なら決定2で足りる**（原因A を構造的に迂回できている） |
| M4 | `IME_KANJI_MARKER` | 起動 | **押下中** | **失敗なら「原因B＝修飾キー文脈」が確定**（追補1/3 の説明が付く） |
...
7. **決定0 の M4（`ime_kanji`/起動/Shift 押下中）は未実施のまま。** M3 成功と
   KeyUp 欠落の発見が優先されたため保留した。M3 が(KeyUp 欠落を除いて)成功
   している以上、原因B（修飾キー文脈の欠落）が実害を持つかは M3 の成功結果
   だけでは判定できない——M3 は Shift を絡めていないため。決定2 の
   synthetic Shift↑ 前置が実際に必要かどうかを確定するには M4 の実施が要る。
