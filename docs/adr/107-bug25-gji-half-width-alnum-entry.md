...
**追記（2026-08-27、決定5を撤回。`docs/known-bugs.md` BUG-25 追補9）:**
実機ソーク中のユーザーから「Composition中もShift単独タップで半角英数に
なってほしい」という明示的な要望を受けた。上記の追記が「決定5を緩和する
条件」として示した「複数回再試行での安定性確認」は、決定0のspikeによる
機械的な再試行ではなく、**ユーザー自身の実利用フローでの確認**という形で
満たされた（追補5の2回の成功に、ユーザーの通常入力での複数回の成功が
積み上がった）。Composition中のブロックによってユーザーが実際に不便を
感じている（"ならない"という明示的な不満）以上、サンプル数を理由に
機能を止め続けるコストの方が、KeyUp欠落由来の潜在的な不安定化リスクより
実害が大きいと判断し、決定5のcomposition/候補ウィンドウブロックを撤去した
（`Output::send_gji_half_width_alnum_toggle`・
`plan_half_width_alnum_action`から`composing`判定を削除）。以後
preedit破壊やモード誤認識の実機報告があれば、ここにガードを復活させる。
