...
1. **なぜ追補1・追補3 で `[hook] IME-mode vk=0xF0` が一度も出なかったのか。**
   `ImeKeyKind::from_vk(0xF0)` は `Some(Alphanumeric)` を返し、当該ログは
   `self_injected` の早期 return より前にあるため、`self_injected=true` として出るはず
   である。同一セッションで 0xF2 は毎回出ていた。候補: (a) awase より後に導入された
   別の LL フックが 0xF0 を先に消費している、(b) `SendInput` 発行のスレッド/文脈に依存する
   何か、(c) 当時のログ収集の取りこぼし。決定0 の各セルで必ず記録する。
   **この疑問が未解決でも決定2 の実装は妨げられない**（フックで観測されるか否かと、
   OS/GJI へ届くか否かは独立）。
