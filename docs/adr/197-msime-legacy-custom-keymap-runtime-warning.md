...
- **旧UIのキー割当てとawaseが競合する一般的なリスクは未解明のまま残る**（ユーザー指摘、
  2026-09-23）。今回確認できたのは「コード`CE`（『IMEオン/オフ』トグル機能、ADR-148の
  実機調査が確認した唯一のIME開閉系機能）は無変換キーには効かない」という狭い事実のみ。
  以下は本ADRの範囲外として未解明のまま残す:
  - 他のプリセット（ATOK/MS-IME2000/VJE/WX、いずれも`msime_legacy_keymap.rs`は`key`
    テーブルの内容を無変換/変換について読んでいるが、コード`CE`以外の値の意味は未解読）。
  - `Ctrl+`/`Shift+`/`Alt+`修飾子付きの無変換/変換への割当て（本モジュールの検出範囲外、
    「このモジュールが検出しないもの」参照）。
  - 実際の設定ツール（`IMJPUEX.EXE`の「ユーザー定義」タブ）経由でユーザーが作成した
    キーマップに、コード`CE`以外の何らかの機能が無変換に割り当てられ、それが実際に
    IME ONを引き起こすケース。ADR-148の「旧UIの機能一覧には単体のIME ON/単体のIME OFF
    という選択肢は存在せず」という前提自体、当時の調査が機能一覧を網羅した保証は無い。
