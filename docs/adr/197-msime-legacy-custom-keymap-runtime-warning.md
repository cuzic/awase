...
id: ADR-197
title: |-
  MS-IME「以前のバージョンのMicrosoft IMEを使う」互換モードの詳細キーカスタマイズについて調査した結果、
  実行時警告は前提が実機で否定されたため撤回。ADR-196向けの互換モードフラグ読み取りのみ採用する
summary: |-
  ...
  トグル割当てが、新UI側の同種検出（`msime_key_assignment.rs`）と違い不具合報告への添付にしか
  使われておらず、`check_and_warn`のような実行時警告が無い、という非対称だった。**しかし2026-09-23、
  GitHub Actions CI（windows-latest）で4パターン（NATURAL基準／Custom最小テーブル／Custom最小
  +互換モードON／dragonflyg4実機の完全な84レコードテーブル+互換モードON）を実機的に検証した
  ところ、いずれも無変換キーはIME OFFのまま変化せず、さらにユーザー本人がdragonflyg4実機で
  物理キーを押して「無変換は直接入力のとき押してもIME ONにならない」と直接確認した。** つまり
  警告の前提（旧UIのトグル割当てが無変換キーの実IME挙動を変える）自体が実機で否定された
  ——round1のopus-adversarial-consultレビュー（B1、変換キーの「ON確認」も既定挙動と区別できない
  という指摘）と合わせ、この競合は実在しないと判断し、実行時警告（決定1〜3）は撤回した。
...
status: |-
  草案rev4(2026-09-23)。**決定1〜3（実行時警告）は撤回・非実装。** CI実機検証4パターン
  （「残された未検証事項」節）がいずれも無変換キーの状態変化を再現できなかったのに加え、
  2026-09-23、ユーザー本人がdragonflyg4実機で直接確認し「無変換は直接入力のとき押いても
  IME ONにならない」と明言（この会話ログが一次情報）。旧UIの「IMEオン/オフ」トグル割当て
  （コード`CE`）が無変換キーの実IME挙動を変えるという、決定1の前提そのものが実機で
  **否定された**。round1レビューB1が指摘した懸念（変換キーの「ON確認」は既定挙動と区別
  できない）と合わせると、この機能（旧UIのキー割当てとawaseの競合）自体が実在しないと
  判断する。決定1〜3は撤回し、実装しない。**決定4（互換モードフラグの読み取りプリミティブ、
  ADR196-T5向け）はこの撤回と無関係に独立して有効**——ADR-196の陳腐化検出はこの前提
  （キーの実効果）に依存しないため影響を受けない。opus-adversarial-consult round1
  （`197-opus-review-round1.md`）のM1〜M5・S1〜S7は決定1〜3が対象だったため未反映のまま
  クローズする。
related_adr:
  - "ADR-148"
  - "ADR-092"
...
押して「直接入力のとき押してもIME ONにならない」と直接確認した。決定1の前提（旧UIのIMEオン/
オフトグル割当てが無変換キーの実IME挙動を実際に変える）は実機で否定された。以下は撤回した
設計をそのまま記録として残す（同種の設計を再検討する際、同じ調査を繰り返さないため）。
**実装しないこと。**
...
- CIとの不一致が生じなかった理由（推測、検証はしていない・する必要も無い）: `imjpuexc.exe`
  ...
  観測がどちらも整合する。
- **本ADRの結論**: 決定1〜3（実行時警告）は撤回する。旧UIのキー割当てとawaseが競合する
  という事象自体が実在しないと判断した。
...
- ON→OFF方向（2〜6列目）の実効性は、決定1自体を撤回したため検証の必要が無くなった
  （ON方向〈1列目〉が否定された以上、旧UIの「IMEオン/オフ」トグル機能は無変換/変換キーの
  awase親指シフト運用に実害を与えないと判断する）。
