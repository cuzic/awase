...
- **旧UIのキー割当てとawaseが競合する一般的なリスクは未解明のまま残る**（ユーザー指摘、
  ...
  **追加検証（2026-09-23、ユーザー指摘「ひらがな/カタカナ/英数等、実際にIME ONを引き起こす
  ...
  これは以下のいずれかを示唆する（優先度順、いずれも未検証）:
  1. **レジストリへの直接書き込み（`Set-ItemProperty`+`ctfmon`再起動）では、実行中の
     MS-IMEエンジンにこのテーブルの変更が反映されない**。実際の設定ツール
     （`IMJPUEX.EXE`の「ユーザー定義」タブ）は、レジストリ書き込みに加えて何らかの
     IPC/COM通知をエンジンへ送っている可能性があり、本ADRの検証手法（レジストリ直接
     書き込み）はこの通知を欠いているため、コードの値に関わらずどれを試しても反映
     されない、という説明。この場合、CE・CA双方の否定的結果は「そのコードが効かない」
     ではなく「検証手法そのものが機能変更を反映できていない」ことになる。
  ...
  **区別する方法**: 実際の設定ツール（`IMJPUEX.EXE`）のUIを通して無変換にひらがな機能を
  割り当て、それでも効かなければ説明2または3、効けば説明1（本ADRの検証手法の限界）が
  正しいと判定できる。GUI自動操作が必要なため本ADRの範囲では実施していない。
