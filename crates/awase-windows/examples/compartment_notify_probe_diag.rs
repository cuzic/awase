//! **`compartment_notify_probe.rs`(ADR-193)の診断用コピー。** 本体は ADR-193 の別セッションの成果物で、ここでは編集していない。
//! CI で本体が「前面化またはフォーカスに失敗」して ABORT し、キーを1件も注入できなかった原因を切り分けるために別名で置いた。
//! 原因: 前面化(SetForegroundWindow 相当)は成功するが、前面化されるとキーボードフォーカスが入力欄からトップ窓へ移り、本体には
//! `WM_SETFOCUS` の処理が無く `focus_on_probe` が false になる(スパイクの窓プロシージャは入力欄へフォーカスを戻す)。
//! 修正案は `tools/e2e/ime_key_matrix/patches/compartment_notify_probe-setfocus.patch`(本体への1アームの追加、未適用)。
//! このコピーは前面化の戦略を順に試して結果を DIAG 行に残す。それ以外(通知の購読・計測)は本体と同じ。
