//! TsfGate — フォーカス変更直後の TSF モード確定前にキーを一時保留するステートマシン。
//!
//! ## 2層構造
//!
//! - **`TsfGateMachine`**: timed-fsm ベースの純粋ステートマシン。
//!   副作用なし。`RawKeyEvent` を保持しない。テスト可能。
//! - **`TsfGate`**: `TsfGateMachine` を内包し、`RawKeyEvent` の保留を追加するラッパー。
//!   Win32 タイマーとの統合は呼び出し元が担当する（`TIMER_TSF_GATE`）。
//!
//! ## 状態遷移
//!
//! ```text
...
//! ## キー保留の範囲
//!
//! PendingWarmup 状態中のみキーを保留する。保留上限 (`HELD_MAX`) を超えると破棄＋ログ警告。
//!
//! ## 保留キーの再処理
//!
...
impl TimedStateMachine for TsfGateMachine {
    ...
    fn on_event(&mut self, event: GateEvent) -> Response<GateAction, GateTimer> {
        ...
        match (self.state, event) {
            ...
            (PendingWarmup | Probing, BypassConfirmed) => {
                self.state = Bypass;
                Response::emit_one(GateAction::DrainHeld)
                    .with_kill_timer(GateTimer::WarmupTimeout)
            }
...
impl TsfGate {
    ...
    /// run_with_prefetched 完了後、TSF モードと確定した場合に呼ぶ。
    ///
    /// PendingWarmup → Probing に遷移し、保留キーを返す。
    /// 呼び出し後に `TIMER_TSF_GATE` を kill すること。
    /// すでに Probing/Ready/Bypass なら空 Vec を返す（500ms ポーリング等で再呼び出しされても safe）。
    ...
    /// run_with_prefetched 完了後、非 TSF モードと確定した場合に呼ぶ。
    ///
    /// → Bypass に遷移し、保留キーを返す。
    /// 呼び出し後に `TIMER_TSF_GATE` を kill すること。
...
impl std::fmt::Debug for TsfGateMachine {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TsfGateMachine")
...
mod tests {
    ...
    fn assert_state(m: &TsfGateMachine, expected: TsfGateState) {
        assert_eq!(
            m.state(), expected,
            "state mismatch: expected {expected:?}, got {:?}", m.state()
        );
    }
    ...
    fn timer_set(resp: &Response<GateAction, GateTimer>) -> bool {
        resp.timers.iter().any(|t| matches!(t, TimerCommand::Set { id: GateTimer::WarmupTimeout, .. }))
    }

    fn timer_killed(resp: &Response<GateAction, GateTimer>) -> bool {
        resp.timers.iter().any(|t| matches!(t, TimerCommand::Kill { id: GateTimer::WarmupTimeout }))
    }
    ...
    fn scenario_c2_timeout_in_probing_is_ignored() {
        ...
        let r = m.on_timeout(GateTimer::WarmupTimeout);
        // Probing 中のタイムアウトは pass_through (状態変化なし)
        assert_state(&m, TsfGateState::Probing);
    ...
    /// D2: 初期状態 Bypass でのキーは通過（PendingWarmup でない）
    #[test]
    fn scenario_d2_bypass_confirmed_idempotent() {
        ...
        // 再度 BypassConfirmed は pass-through（すでに Bypass）
        let r = m.on_event(GateEvent::BypassConfirmed);
        r.assert_pass_through();
        assert_state(&m, TsfGateState::Bypass);
    }
    ...
    fn gate_wrapper_on_warmup_timeout_drains_held() {
        use awase::types::{KeyClassification, KeyEventType, ScanCode, VkCode};
        use awase::types::RawKeyEvent;

        let mut gate = TsfGate::new();
        gate.machine.on_event(GateEvent::FocusChange); // PendingWarmup へ

        // held に疑似キーを追加（Passthrough = 修飾/Fキー用の分類を流用）
