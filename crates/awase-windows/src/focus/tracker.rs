...
impl FocusTracker {
    ...
    /// （BUG-78 対策、`runtime/focus_tracking.rs` から呼ぶ）。ADR-195段階1の
    /// 学習プロセス（`awase-keymap-learn-win.exe`）も、注入キーを変換しないよう
    /// 恒久バイパス対象として扱う。
    pub(crate) fn is_app_disabled(&self) -> bool {
        self.overrides.is_app_disabled(&self.current.process_name)
            || crate::state::app_suppression::is_keymap_learn_process_name(
                &self.current.process_name,
            )
    }
