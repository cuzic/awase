...
/// `[[keymap]]` の `to` ターゲット送信（ADR-114 決定3）。
///
...
///
/// `target_vk` の Down/Up を同一 `SendInput` バッチで送信する（描画前に完結させ、
/// 中間状態を外部に見せない。Chrome cold-start 検出の VK_A+BS アトミックバッチと
/// 同じ原則）。マーカーは `INJECTED_MARKER`（`ime.rs` の3箇所が使う
