...
// D0/D2/D4 に対応する実機診断コード一式。Windows Terminal + GJI で awase
// Engine 有効時に物理半角/全角キーを押すと余分な「@」が出る問題（BUG-113）
// の恒久修正候補を1回の実機スパイクでまとめて検証する。
//
// `ime_controller.rs::GjiDirectStrategy::apply` から呼ばれる。本モジュール
// （`ime_controller.rs`）は観測値を自ら読んではいけないという既存の
// アーキテクチャ制約があるため、物理修飾キー状態の読み取り・実際の
// `SendInput` 構築はすべてここ（`ime.rs`）に置く。
//
// 調査終了後は `src/config.rs::diag_bug113_mode_cycle_enabled`・
// `ime_controller.rs` の呼び出し元・本ブロック一式を削除すること。

/// BUG-113 診断専用（一時的）: 実際に Ctrl/Shift が押下中かどうか。
///
/// `send_ime_mode_key` が自然に4イベント（またはそれ以上）のバッチを送る
/// 状況（Ctrl+無変換 等）では、診断モードを割り込ませず通常経路
/// （＝ D0-1 が読みたい「ありのままの」バッチ）に任せる。ADR-133 D1 の
/// 条件（`held_skip_alt.ctrl == false && held_skip_alt.shift == false`）と
/// 同じ判定をここでも行う。
#[must_use]
pub(crate) fn diag_bug113_real_modifiers_held() -> bool {
    use crate::vk::{VK_LCONTROL, VK_LSHIFT, VK_RCONTROL, VK_RSHIFT};
...
/// BUG-113 診断専用（一時的）: D0-3 用トリガー判定。
///
/// Alt は `send_ime_mode_key` のバッチ形状に一切影響しない
/// （`held_skip_alt.alt` は常に false）ため、Alt 押下中の物理半角/全角
/// キーを「D0-3（`KanjiToggleStrategy` が単独で `p` を混入させるか）」の
/// 専用トリガーとして安全に転用できる。
#[must_use]
pub(crate) fn diag_bug113_should_run_kanji_probe() -> bool {
    use crate::vk::{VK_LMENU, VK_RMENU};
    crate::hook::is_physical_key_down(VK_LMENU) || crate::hook::is_physical_key_down(VK_RMENU)
}

/// BUG-113 診断専用（一時的、候補V）: `[vk↓]`/`[vk↑]` を2回の独立した
/// `SendInput` に分割して送信する。ADR-133 D2 候補V。新しい VK も偽の
/// 修飾キーも持ち込まない第一候補——down/up を分割することで「1回の
/// `SendInput` に down/up が同居していること」自体が「@」の引き金かを
/// 確かめる。
///
/// `sent != inputs.len()` の警告は2回の呼び出しそれぞれで独立に行う
/// （round3 レビュー N4: 2回目だけ失敗すると `vk` が論理的に down のまま
/// 残るため、両方の戻り値を個別に検査する）。
pub(crate) fn diag_bug113_send_vk_ime_off_split(vk: awase::types::VkCode) -> bool {
    use crate::tsf::output::{make_key_input_ex, IME_KANJI_MARKER};
    let down = [make_key_input_ex(vk, false, IME_KANJI_MARKER)];
    let sent_down = crate::win32::send_input_safe(&down);
    if sent_down as usize != down.len() {
        log::warn!("[bug113-diag] mode=V(split): down sent {sent_down}/1");
    }
    let up = [make_key_input_ex(vk, true, IME_KANJI_MARKER)];
    let sent_up = crate::win32::send_input_safe(&up);
    if sent_up as usize != up.len() {
        log::warn!("[bug113-diag] mode=V(split): up sent {sent_up}/1");
    }
    log::warn!(
        "[bug113-diag] mode=V(split): vk=0x{vk:02X} を2回のSendInputに分割 \
         (down sent={sent_down}/1, up sent={sent_up}/1, split=true)"
    );
    sent_down as usize == down.len() && sent_up as usize == up.len()
}

/// BUG-113 診断専用（一時的、候補A）: 対象 VK 自身を同一バッチ内で2度
/// 打鍵する自己エコーパディング。ADR-133 D2 候補A（第二候補）。
///
/// `vk↓(decoy) vk↑(decoy) vk↓(real) vk↑(real)` の4イベントバッチ。
/// 「open 軸では冪等」という既存の前提が、この4イベント形状でも
/// 「@」を抑止できるかは実機で未検証（ADR-133 Blocker 3 参照）。
pub(crate) fn diag_bug113_send_vk_ime_off_self_echo(vk: awase::types::VkCode) -> bool {
    use crate::tsf::output::{make_key_input_ex, IME_KANJI_MARKER};
    let inputs = [
    ...
    let sent = crate::win32::send_input_safe(&inputs);
    log::warn!(
        "[bug113-diag] mode=A(self-echo): vk=0x{vk:02X} を同一バッチ内で2度打鍵 \
         sent={sent}/{}",
        inputs.len()
    );
...
/// BUG-113 診断専用（一時的、候補B3）: 偽 Ctrl release ブラケット（3イベント、
/// 末尾に偽の KeyDown を送らない版）。ADR-133 D2 候補B の採用形。
///
/// `Ctrl↑(fake) vk↓ vk↑` の3イベント。実 Ctrl が押されていない状況で
/// `Ctrl↑` だけを送っても、対になる KeyDown が無い＝OS 側のキー状態は
/// 変化しない（stuck Ctrl が構造的に発生しない）。v1/v2 の対称形案
/// （round2 レビューで Ctrl 押下中に vk を配送してしまうと判明し撤回済み）
/// とは異なり、vk は Ctrl 非押下のまま届く。
pub(crate) fn diag_bug113_send_vk_ime_off_fake_ctrl_3event(vk: awase::types::VkCode) -> bool {
    use crate::tsf::output::{make_key_input_ex, IME_KANJI_MARKER};
    use crate::vk::VK_CONTROL;
    let inputs = [
        make_key_input_ex(VK_CONTROL, true, IME_KANJI_MARKER),
        make_key_input_ex(vk, false, IME_KANJI_MARKER),
        make_key_input_ex(vk, true, IME_KANJI_MARKER),
    ];
    let sent = crate::win32::send_input_safe(&inputs);
    log::warn!(
        "[bug113-diag] mode=B3(fake-ctrl-3event): 偽Ctrl↑ + vk=0x{vk:02X} sent={sent}/{}",
        inputs.len()
    );
    sent as usize == inputs.len()
}

/// BUG-113 診断専用（一時的、候補B4）: 偽 Ctrl release ブラケット（4イベント、
/// 末尾も KeyUp にする版）。候補B3 で「@」が消えない場合の予備。
///
/// `Ctrl↑(fake) vk↓ vk↑ Ctrl↑(fake)` の4イベント。末尾も KeyUp のため、
/// KeyUp の重複（`send_ime_mode_key_with_shift_release_prefix` の左右
/// Shift 二重 KeyUp 送信と同じ根拠で無害）だけで stuck Ctrl を作らない
/// まま4イベントバッチを再現する。
pub(crate) fn diag_bug113_send_vk_ime_off_fake_ctrl_4event(vk: awase::types::VkCode) -> bool {
    use crate::tsf::output::{make_key_input_ex, IME_KANJI_MARKER};
    use crate::vk::VK_CONTROL;
    let inputs = [
        make_key_input_ex(VK_CONTROL, true, IME_KANJI_MARKER),
    ...
    let sent = crate::win32::send_input_safe(&inputs);
    log::warn!(
        "[bug113-diag] mode=B4(fake-ctrl-4event): 偽Ctrl↑ + vk=0x{vk:02X} + 偽Ctrl↑ \
         sent={sent}/{}",
        inputs.len()
    );
    sent as usize == inputs.len()
