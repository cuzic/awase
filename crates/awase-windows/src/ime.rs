...
///
/// **ADR-133 BUG-113 改修は実機A/Bで反証・revert 済み（2026-09-05）。** 一時的に
/// `wVk` を保持したまま `MapVirtualKeyW` 実測 scan を `wScan` にも埋める
/// （`make_scan_key_input()` 相当）変更を試したが、Windows Terminal + GJI で
/// 実機検証した結果「@」の再現に変化が無く、この関数の `wScan=0` は真因では
/// なかった。真因は drift correction が `FeedbackPolicy::Read` を使って
/// `VK_IME_OFF` を無制限に近い頻度で再送し続けることと判明した
/// （`docs/known-bugs.md` BUG-114 参照、`docs/experiments.md` エントリ20）。
/// 本関数は元の `wScan=0` 固定に戻している。
///
/// # Safety
/// Win32 API を呼び出す。メインスレッドから呼ぶこと。
#[must_use]
