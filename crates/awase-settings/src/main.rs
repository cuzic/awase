...
impl SettingsApp {
    ...
    fn tab_advanced(&mut self, ui: &mut egui::Ui) {
        ...
        ui.checkbox(
            ...
            "GJI（Google 日本語入力）の無変換/変換/ひらがな/カタカナキーの状態依存トグルに追従する",
        )
        .on_hover_text(
            "ON(既定)の場合、GJIのキーマップ設定（ATOKプリセット、または\n\
             カスタムキーマップでの同種の割当て）が無変換/変換/ひらがな/\n\
             カタカナキー単体に状態依存のIME ON/OFFトグルを割り当てていれば、\n\
             awaseがIMEの開閉を代わりに行い、Engineも追従します。OFFにすると\n\
             awaseはそれに追従せず、ログで警告のみ行います（生キーはGJIに\n\
             届くので実IMEは開閉しますが、awaseのEngineは追従しません）。\n\
             この種のトグルは非冪等（誤って発火すると意図せずIME状態が\n\
             反転する）ため、awaseが唯一の変更主体になります。\n\
