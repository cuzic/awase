...
fn run_with_fallback_impl(
    ...
    let glow_result = eframe::run_native(
        app_name,
        glow_options,
        Box::new(move |cc| Ok(creator(cc))),
    );
...
fn select_wgpu_adapter(
    ...
    let compatible = |adapter: &&wgpu::Adapter| surface.is_none_or(|s| adapter.is_surface_supported(s));
...
fn env_report_lines(diag: &EnvDiagnosis) -> Vec<String> {
    vec![
        ...
        format!(
            "グラフィックスアダプタ: {}",
            diag.primary_adapter.as_deref().unwrap_or("取得できませんでした")
        ),
        format!(
            "opengl32.dll の読み込み: {}",
            if diag.opengl32_loadable { "成功" } else { "失敗" }
        ),
    ]
}
...
fn is_fallback_adapter_name(name: &str) -> bool {
    ...
    lower.contains("basic render") || lower.contains("basic display") || lower.contains("remote display")
}
...
fn primary_display_adapter_name() -> Option<String> {
    ...
    use windows::Win32::Graphics::Gdi::{
        DISPLAY_DEVICEW, DISPLAY_DEVICE_ATTACHED_TO_DESKTOP, EnumDisplayDevicesW,
    };
