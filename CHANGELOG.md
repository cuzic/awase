...
- **`config.toml` の間違いを黙って無視せず、警告として知らせるようにした（ADR-201 段階2）。旧表記 `[[keymap]]` が効くようになる**
  - 文書や過去の記述にある `[[keymap]]`（実際の項目名は `[[keymaps]]`）は、今までは無言で読まれず、キー割り当てが効かなかった。**更新すると `[[keymap]]` の割り当てが効き始める**（`[[keymaps]]` と両方あれば連結して読む）。心当たりがあれば `config.toml` を確認すること
...
- **句読点（「。」「、」）を打った瞬間にIMEの入力を確定するレイアウト `nicola_kakutei.yab` を追加した**
  - 設定画面の「レイアウト」から選ぶだけで使える（`nicola_keytop.yab` と「。」「、」の2キー以外は同一）。やまぶき／DvorakJの「句読点で確定」と同じ仕組みを、打鍵列構文（`。+CV4D`）で実現している
  - 確定にはIME側で Ctrl+M が「全確定」になっている必要がある。他のアプリで Ctrl+M に別の機能（Wordの改行スタイル変更など）が割り当てられている場合は、そちらの割り当てを外すか、このレイアウトを使わないこと
  - 実機（Google 日本語入力・Microsoft IME）で、「。」を打つと変換中の文字列が確定されることを確認した（標準の編集コントロールでの確認。他のアプリでの動作は個別には確認していない）
- **設定画面の「上級者向け設定」タブに、打鍵列構文（`keystroke_sequence`）のON/OFFチェックボックスを追加した**

### 変更

- **打鍵列構文（配列ファイルの `CV4D` や `+` 区切り、`@マクロ名` などの記法）の解釈を、既定で有効にした**（従来は既定でOFF）
  - 読み込む全ての `.yab` に影響する。これらの記法は偶然一致するには特殊なため実害は小さいと判断したが、`.yab` を独自編集している場合は内容を確認すること。無効にしたい場合は設定画面の「上級者向け設定」または `config.toml` の `keystroke_sequence = "off"`
- **MSIをアンインストールしても、`config.toml` と配列ファイル（`.yab`）を削除しないようにした**（ZIP版と同じ扱い）
  - 従来はアンインストールでこれらのカスタマイズ内容が消えていた。アンインストール後もファイルが残るようになる。再インストールでは既存のファイルをそのまま使う
- **不具合報告に添付する入力履歴の記録で、キーの自動リピートによって記録枠が埋まってしまわないよう、連続リピートを畳み込むようにした**（内部の診断機能の改善）

### 修正

- **Google 日本語入力で、Ctrl+無変換 を2回目以降に押してもIME OFFにならず、そのまま入力が候補ウィンドウに出てしまうことがある不具合を修正**
  - 症状: Microsoft Edge など（`Chrome_WidgetWin_1`）で、Ctrl+無変換 の直後に文字を打つと変換候補が出て、Ctrl+無変換 を押し直しても何も送られずIME OFFにならなかった
  - 原因: 「すでにIME OFFになっているはず」という内部の判断が、実際には候補ウィンドウが表示されている（IMEの状態と食い違っている）という証拠を無視して、キー送信を省略していた
  - 修正: 候補ウィンドウの表示を検知していた場合は「一致している」と判断せず、キーを送信するようにした
- **Google 日本語入力で、キーマップのプリセットが「カスタム」以外に設定されていると、実在する `custom_keymap_table`（変換キーを IME ON に割り当てた設定など）が無視され、変換キーを単独タップしてもNICOLAエンジンが有効にならない不具合を修正**
  - 症状: Windows Terminal + Google 日本語入力で、直接入力状態から変換キーを単独タップするとGoogle 日本語入力自身はひらがなに切り替わるが、awase側がそれを認識せずNICOLA配列が効かないままだった（実機で確認）
  - 修正: `custom_keymap_table` に該当する行があれば、キーマップのプリセットの値に関わらずその内容を優先するようにした
- **配列ファイル（`layout/`）や `config.toml` が無い環境で、awase が起動してもNICOLA配列が使えない（Engineが動かない）不具合を修正**
  - 症状: `layout/` を手動で削除した環境や、旧版のMSIをアンインストールしてから再インストールした環境などで、起動時に「レイアウトディレクトリが見つかりません」と警告され、NICOLA入力ができなかった
  - 修正: 起動時に、`config.toml` が無ければ既定の内容で、配列ファイル（`.yab`）が1本も無ければ同梱の6本を、それぞれ生成するようにした（既にあるファイルには一切触らない）
...
- **タスクトレイの「学習キャッシュをクリア」メニュー項目をクリックしても何も起きない不具合を修正**（注: 実際には未修正だった。BUG-108 参照）
